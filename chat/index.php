...
if($datauser[chwhy]==""){echo"<div>Причина не указана</div>";}else{echo"Причина:<font color='".$cdinf."'> $datauser[chwhy]</font><br/>";}
...
echo "<a href='index.php?act=delpost&amp;id=".$id."&amp;yes'>Удалить</a>|<a href='index.php?id=".$ms[refid]."'>Отмена</a><br/>";}}else{
...
echo "<a href='index.php?act=room&amp;id=".$id."&amp;yes'>Да</a>|<a href='index.php?id=".$id."'>Нет</a><br/>";}}else{
echo "Доступ закрыт!!!<br/>";}
break;
############
case "moders":
...
if (!empty($_GET['id'])){$id=intval(check($_GET['id']));}
...
if ($login!=$mod1[name]){
echo "<a href='../str/anketa.php?user=".$mod1[id]."'><font color='".$conik."'>$mod1[name]</font></a>";}else{
...
if(empty($datauser[postchat])){$fpst=1;}else{
...
if ($datauser[sex]=="m"){
$tx="молодец! Ты угадал правильный ответ:  $answer за $otvtime секунд $pods ,и заработал $bls баллов. Всего правильных ответов:<b>$itg</b>, твой игровой баланс $balans баллов.";}else{$tx="молодец! Ты угадала правильный ответ:  $answer за $otvtime секунд $pods ,и заработала $bls баллов. Всего правильных ответов:<b>$itg</b>, твой игровой баланс $balans баллов.";}
$mtim=$realtime+1;
...
require("../incfiles/head.php");
require("../incfiles/inc.php");
echo "Добавление сообщения в комнату <font color='".$cntem."'>$type1[text]</font>(max. 500):<br/><form action='index.php?act=say&amp;id=".$id."' method='post'><textarea cols='40' rows='3' title='Введите текст сообщения' name='msg'></textarea><br/>";
if ($offtr!=1){
...
#######################
header ("location: index.php?id=$th");}else{
...
if (empty($udat[nastroy])){$nstr="без настроения";}else{$nstr=$udat[nastroy];}
...
if ($dostkmod==1){
echo "<a href='../".$admp."/zaban.php?user=".$udat[id]."&amp;chat&amp;id=".$id."'>Банить</a><br/>";}elseif ($dostcmod==1){
...
if ($conts==0){echo "<a href='../str/cont.php?act=edit&amp;nik=".$udat[name]."&amp;add=1'>Добавить в контакты</a><br/>";
...
if ($ignss==0){echo "<a href='../str/ignor.php?act=edit&amp;nik=".$udat[name]."&amp;add=1'>Добавить в игнор</a><br/>";
...
echo "<a href='index.php?id=".$mass[id]."'><font color='".$cntem."'>$mass[text]</font></a> (";wch($mass[id]);echo ")<br/>";
}
echo "<hr/>";

echo "<a href='who.php'>Кто в чате(";wch();echo ")</a><br/>";
...
require("../incfiles/head.php");
require("../incfiles/inc.php");
echo "<form action='index.php?act=pass&amp;id=".$id."' method='post'><br/>Введите пароль(max. 10):<br/><input type='text' name='parol' maxlength='10'/><br/><input type='submit' name='submit' value='Ok!'/><br/></form><a href='index.php'>В чат</a><br/>";require("../incfiles/end.php");exit;}}
...
a:hover { text-decoration: none; font-size: 12px; color : #FF6600 }

</style>
      </head>
      <body>";

echo "<div class = 'a' ><center><b>$textl</b></center></div><div class = 'a' >";

require("../incfiles/inc.php");


echo "<b><font color='".$cntem."'>$type1[text]</font></b><br/>";
if ($type1[dpar]=="in"){echo "<a href='index.php?act=chpas&amp;id=".$id."'>Сменить пароль</a><br/>";}
...
if ($offtr!=1){
    echo "<input type='checkbox' name='msgtrans' value='1' /> Транслит сообщения<br/>";}
echo "<input type='submit' title='Нажмите для отправки' name='submit' value='Отправить'/><br/></form>";}else{
...
if ($colmes < $start + $chmes){ $end = $colmes; }
else {$end = $start + $chmes; }
...
echo "<a href='index.php?act=say&amp;id=".$mass[id]."'><b><font color='".$conik."'>$mass[from]</font></b></a> ";}else{
echo "<b><font color='".$csnik."'>$mass[from]</font></b>";}}else{
...
if ($pfon==1){
echo "</div>";}
if (!empty($mass[nas])){echo "<font color='".$cdinf."'>$mass[nas]</font><br/>";} 
if ($mass[dpar]==1){echo "<font color='".$clink."'>[П!]</font>";}
if (!empty($mass[to])){

if ($mass[to]==$login){echo "<font color='".$cdinf."'><b>";}
echo "$mass[to], ";
if ($mass[to]==$login){echo "</b></font>";}
}
...
if ($colmes > $start + $chmes) {echo ' <a href="?id='.$id.'&amp;page='.($page+1).'">&gt;&gt;</a>';}}
echo "<hr/><a href='who.php?id=".$id."'>Кто здесь?(";wch($id);echo ")</a><br/>";
echo "<a href='who.php'>Кто в чате(";wch();echo ")</a><br/>";
...
require("../incfiles/head.php");
require("../incfiles/inc.php");echo "Вы не авторизованы!<br/><a href='../in.php'>Вход</a><br/>";}
