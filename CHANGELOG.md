...
#### Added
- Роутер: именованные маршруты в формате `module.action`.
- Роутер: автоматическое обнаружение маршрутов модулей из `modules/*/config/routes.php`.
- Загрузки: Заменен редактор BB-кодов на полноценный CKEditor в формах добавления, редактирования и импорта файлов. Для миграции существующих описаний из BB-кодов в HTML добавлена одноразовая консольная команда `downloads:convert-bbcode`.
- Библиотека: Заменен редактор BB-кодов на полноценный CKEditor при создании и редактировании статей. Для миграции существующих текстов статей из BB-кодов в HTML добавлена одноразовая консольная команда `library:convert-bbcode`.
- Комментарии (загрузки, библиотека, фотоальбомы): Заменен редактор BB-кодов на CKEditor. Для миграции существующих комментариев из BB-кодов в HTML добавлены одноразовые консольные команды `downloads:convert-comments`, `library:convert-comments` и `album:convert-comments`.
...
- Добавлен модуль «Коллекции» (`collections`): произвольные коллекции с разделами, настраиваемыми полями и элементами, управление через админку. При обновлении с предыдущей версии таблицы модуля создаются скриптом `install/install_collections.php`.
- Добавлен модуль «Согласия» (`consent`): справочник согласий на обработку персональных данных (контекст, язык, заголовок, HTML-текст, версия, флаги обязательности и активности) и журнал принятых согласий (`user_id`, `consent_id`, версия, IP, дата). Управление через админку (`/admin/consents`, редактор CKEditor) и журнал (`/admin/consents/log`), публичный просмотр текста по `/consent/{id}`. На форме регистрации выводятся снятые по умолчанию чекбоксы активных согласий контекста `register` со ссылкой на текст; без отметки обязательных согласий регистрация не проходит, а факт принятия пишется в журнал на сервере. Предоставлен программный сервис `ConsentService` (получение активных согласий по контексту с учётом языка и запись в журнал). При обновлении с предыдущей версии таблицы модуля создаются скриптом `install/install_consent.php`.
- Cookie-баннер и согласие на аналитику: в модуль «Согласия» добавлена страница «Cookie-баннер» (`/admin/cookie-banner`) — включение баннера, мультиязычный текст и числовая версия для повторного показа согласия всем; настройки хранятся в `config('johncms')`. На публичных страницах темы `default` выводится всплывающий баннер с кнопкой «Принять» (согласие сохраняется в `localStorage` с учётом версии); при пустом тексте показывается переводимый текст по умолчанию со ссылкой на «Политику конфиденциальности». У счётчиков аналитики добавлен флаг «Загружать только после согласия на cookie» — такие счётчики подключаются на странице лишь после принятия баннера (до согласия их код изолирован в `<template>` и не выполняется). В форму счётчика также добавлен чекбокс включения/выключения. При обновлении с предыдущей версии столбец `require_cookie_consent` в таблице `cms_counters` добавляется скриптом `install/update_counters_cookie_consent.php`.
- Добавлен модуль «Контакты» (`contacts`): публичная страница `/contacts/` с контактной информацией (email, телефон, адрес, часы работы, ссылки на соцсети и мессенджеры) и микроразметкой Schema.org, а также формой обратной связи. Контактные данные настраиваются в админке (`/admin/contacts`) и хранятся в `config('johncms')`; текстовые поля мультиязычные — при отсутствии значения для текущего языка используется язык сайта по умолчанию, затем английский. Обращения сохраняются в таблицу `contact_messages` и доступны в админке (`/admin/contacts/messages`) со списком, фильтром по статусу («Новые» / «Обработанные»), просмотром, отметкой об обработке и удалением; уведомление о новом обращении ставится в очередь писем и уходит на указанный в настройках e-mail (по умолчанию — e-mail сайта). Форма защищена капчей для гостей, скрытым honeypot-полем, проверками CSRF, флуда и бана, а также запретом повторной отправки одинакового текста с одного IP; на форме выводятся чекбоксы согласий контекста `contacts` из модуля «Согласия» с записью в журнал. При обновлении с предыдущей версии таблица модуля создаётся скриптом `install/install_contacts.php`.
- Форум: Добавлена одноразовая консольная команда `forum:normalize-message-links` для нормализации устаревших ссылок в текстах постов: снимает обёртку `/redirect/` с внутренних ссылок и переписывает legacy-адреса на канонические (`?act=show_post&id=N` → `/forum/post/N/`, `?type=topic&id=N` → ЧПУ темы). Внешние ссылки не трогаются.
- Согласия: В заголовке согласия теперь разрешён HTML — в него можно вставлять ссылки на другие страницы (например, «Ознакомлен с [Публичной офертой](/offer/) и даю [Согласие на обработку персональных данных](/about/docs/personal_processing/)»). Заголовок хранится сырым и очищается при выводе по списку допустимых инлайновых тегов (`a`, `b`, `strong`, `i`, `em`, `u`, `br`), а в заголовке страницы, хлебных крошках и таблицах админки показывается без разметки. Поле «Текст» стало необязательным: если оно пустое, у согласия нет собственной страницы (`/consent/{id}` отдаёт 404), а в форме выводится только заголовок со своими ссылками — такое согласие работает как хранилище формулировки. Если у согласия есть текст и в заголовке нет собственных ссылок, заголовок по-прежнему оборачивается в ссылку на страницу с текстом.
- Админка: Добавлена страница «Обслуживание» (`/admin/maintenance`, права ≥ 9) для запуска консольных команд из панели. Команды помечаются атрибутом `#[AsAdminTask]`: быстрые (например, `cache:clear`) выполняются синхронно в запросе, а долгие фоновые (например, `sitemap:generate`) ставятся в файловую очередь и подхватываются планировщиком через новую команду `admin-tasks:run-queued`. Состояние и блокировки хранятся в `data/admin_tasks` (вне кэша, чтобы `cache:clear` их не удалял).

#### Fixed
- Загрузки: Исправлена обработка путей при сканировании директорий и очистке осиротевших файлов.
- Админка: После сохранения настроек изменения не применялись сразу из-за кэша DI-контейнера. Теперь кэш контейнера сбрасывается автоматически (#252).
- Страница 404: Текст ошибки не переводился. Сообщение бралось из текущего домена переводов (домена модуля, обрабатывающего запрос), где его нет, а заголовок `ERROR: 404 Not Found` вообще не проходил через перевод. Теперь обе строки берутся из системного домена; добавлен перевод заголовка.

#### Changed
- Загрузки: Модуль значительно переписан с интеграцией в роутер, добавлены slug-based URL для разделов и файлов; 301-редиректы с устаревших `?id=` адресов. Для генерации slug-ов из названий существующих разделов и файлов добавлена одноразовая консольная команда `downloads:generate-slugs`.
- Библиотека: Модуль значительно переписан с интеграцией в роутер, добавлены чистые URL на основе slug-ов для разделов и статей; улучшен поиск (полнотекстовый с защитой от XSS и операторных инъекций). Для генерации slug-ов из названий существующих разделов и статей добавлена одноразовая консольная команда `library:generate-slugs`.
- Личные сообщения: Модуль значительно переписан на новую слоистую архитектуру с переводом на контроллеры и полноценной интеграцией с роутером, избавлен от устаревших инклудов и legacy-кода.
- Авторизация и регистрация: Модули переведены на контроллерную архитектуру с use cases.
- Уведомления, Редиректы: Модули переведены на новую слоистую архитектуру.
- Смена языка: Модуль переведён на контроллерную архитектуру.
- Кто на сайте: Модуль переведён на PSR-4 архитектуру с контроллерами через DI.
...
#### Added
- Поддержка invokable контроллеров

#### Fixed

#### Changed
- Контроллеры теперь не наследуются от Johncms\Controller\BaseController. Вместо этого используется Johncms\Http\Controller\ControllerContext для инициализации модульных шаблонов и локализаций.
- Заменен dependency injection контейнер на Symfony DI.
- Форум: Заменен редактор BB-кодов на полноценный CKEditor.
- Форум: Добавлена возможность вставки изображений в текст поста.
- Форум: Добавлено ЧПУ для разделов, тем форума.
- Форум: Модуль значительно переписан для полноценной интеграции с роутером и избавления от инклудов.

## 9.7 - 2025-12-21

#### Added
- Обновлен bootstrap до версии 5.3.
- Добавлена тёмная тема.
- Для отправки email теперь используется symfony/mailer.
- Улучшено логирование ошибок. Все ошибки теперь отлавливаются и сохраняются в data/logs с автоматической ротацией файлов.

#### Fixed
- В библиотеке исправлены ошибки в запросах при включенном строгом режиме и ONLY_FULL_GROUP_BY
- В привате исправлены ошибки в запросах при включенном ONLY_FULL_GROUP_BY

#### Changed
- Минимальная версия php - 8.2
- Внесены различные изменения для совместимости с php 8.2-8.4
- Удалена библиотека laminas/laminas-mail т.к. пакет заброшен и больше не поддерживается.
- Обновлены прочие библиотеки до актуальных версий на сколько это возможно на данный момент.
