...
- **У модулей появился реестр.** Система больше не считает модулем всё, что лежит в папке: модуль описывает себя манифестом `module.php` (ключ `вендор/модуль`, короткое имя `alias`, название), а сайт хранит в `config/autoload/modules.local.php`, что у него установлено и что выключено.
  ...
  Установку, обновление и удаление модуля можно выполнить в фоне: на экране подтверждения есть галочка «выполнить в фоне», задача уходит планировщику и запускается в течение минуты. Это для модулей с тяжёлыми миграциями, которые не успевают отработать за время веб-запроса. Фоновые задачи научились принимать аргументы (`Johncms\AdminTasks`), а задача, которой нужен аргумент, не показывается на странице «Обслуживание» — её ставит в очередь тот экран, где этот аргумент есть.

  Модуль можно поставить через composer: пакет с `"type": "johncms-module"` ставится обычным `composer require`, и CMS находит его в `vendor/` — плагин для раскладки файлов не нужен, система спрашивает у composer, какие пакеты этого типа установлены. После установки пакета composer печатает, что делать дальше: `module:install <вендор>/<модуль>`. При удалении пакета — предупреждение, что таблицы и данные модуля останутся, и подсказка сначала выполнить `module:uninstall --purge`.

  Модуль можно поставить из zip-архива: `module:install --from=<архив.zip>`. Где окажется модуль, решает ключ в его манифесте, а не имя папки в архиве. Архив сначала распаковывается во временный каталог, прежняя версия переносится в `data/backups/`, и только потом новая занимает её место — сорванная установка оставляет сайт таким, каким он был. Архив модуля, который уже установлен, считается обновлением.

  Перед распаковкой архив проверяется: пути, ведущие за пределы модуля (`../`), абсолютные пути, символические ссылки, слишком большой распакованный размер, отсутствие `module.php` и больше одного каталога в корне — всё это отклоняется с объяснением, до того как на диск попадёт хоть один байт. Системный модуль из архива заменить нельзя.

  **Раздел модуля в панели администратора объявляется маршрутом.** У маршрута и группы появился `->adminArea()`: он даёт странице переводы панели и первую ссылку в цепочке навигации. Раньше это делали гварды модуля `admin`, поэтому раздел модуля, защищённый собственным правом (новости, коллекции), получал меню панели на английском. Авторам модулей: пометьте группу своих админ-маршрутов — вложенная группа объявляет пометку сама, как и middleware.
...
- **Оборвавшаяся установка модуля больше не оставляет сайт с включённым полумодулем.** Запись о модуле создаётся до миграций — иначе мигратор не найдёт его каталог, — и раньше при падении миграции она оставалась как есть: модуль числился установленным и включённым, кэш контейнера сбрасывался, и следующий запрос собирал контейнер вместе с его сервисами и маршрутами поверх половины схемы. Повторить установку было нельзя: система отвечала «модуль уже установлен», и выйти можно было только через удаление.

  Теперь запись помечается как незавершённая установка. Такой модуль виден в разделе модулей со статусом «Установка не завершена» и объяснением, но не загружается: его сервисы, маршруты и шаблоны в сайт не попадают. Миграции при этом остаются доступны — именно через них установку доводят до конца, повторив `module:install` или нажав «Установить» ещё раз. Пометка снимается, как только отработал установщик модуля; сбой на публикации ассетов или демо-данных её уже не возвращает.

  Включить, выключить или обновить такой модуль система откажется. Удалить — разрешит: это выход, когда доустановить не получается.

  В файле `config/autoload/modules.local.php` появляется ключ `'installing' => true`, и только пока установка не завершена: на сайте, где всё прошло гладко, файл выглядит ровно так же, как раньше. Файл от предыдущей версии читается без изменений.

- **Модуль не может занять имя, которое держит ядро.** Проверка уникальности алиаса сравнивала его только с другими модулями, а часть имён принадлежит не модулям: `system` — это источник миграций CMS и домен её переводов, `theme` — пространство имён шаблонов темы. Модуль с алиасом `system` проходил проверку, после чего его миграции сливались с историей ядра под одним именем, и удаление такого модуля с очисткой данных означало откат схемы всего сайта.

  Теперь занятые имена объявляют сами держатели, а реестр помечает такой модуль как сломанный и не загружает его; установка отклоняется с сообщением, что именно занимает имя. Список не захардкожен — он собирается из констант тех подсистем, которые эти имена держат, поэтому не разъедется при переносе кода в модули.

  `admin` в список не входит: `@admin` — это шаблоны темы и модуля админки, объединённые намеренно, и имя держит модуль, с которым справляется обычная проверка.

  Отдельно закрыта причина, по которой это было опасно: `MigrationLocator` больше не принимает два источника миграций с одним именем и отказывается собирать список до чтения хотя бы одного файла. Проверка не зависит от того, какие модули входят в поставку, и ловит любой сторонний `MigrationSourceProviderInterface`, назвавшийся чужим именем.

- **Модуль подключается к точкам расширения одним интерфейсом.** Раньше по интерфейсу подхватывались только десять точек: аутентификация, права, политика HTML, капча, конвейер контента, источники миграций и пункты меню. Остальные пять требовали, чтобы модуль повторил регистрацию у себя в `config/services.php` — правилом `instanceof` или ручным `->tag(...)`.

  Теперь по реализованному интерфейсу подхватываются и они: консольная команда (`Symfony\...\Console\Command`), провайдер адресов sitemap (`SitemapUrlProviderInterface`), Twig-расширение (`Twig\Extension\ExtensionInterface`), фабрика правила валидации (`RuleConstraintFactoryInterface`) и сервис, который ядро сбрасывает между запросами (`ResetInterface`).

  Последний был самым неприятным: правило `instanceof` в файле ядра до сервисов модуля не доставало, поэтому сервис модуля с состоянием запроса между запросами не очищался — в обычном FPM это незаметно, а в worker-рантайме означает данные одного посетителя, показанные другому.

  Авторам модулей: строки вида `$services->instanceof(Command::class)->tag('johncms.console_command');` и `->tag('johncms.sitemap_provider')` из `config/services.php` можно удалить. Условие одно — класс должен быть объявлен с `autoconfigure()`: он есть у блока `load()`, а вот отдельной строке `set()` его нужно добавить явно, иначе тег не проставится и сервис молча не спросят ни о чём.

  Исключение — почта и установщик: у них свои окружения Twig, и расширение для них по-прежнему помечается вручную тегом `johncms.twig_extension.mail` или `johncms.twig_extension.install`.

- **Изображения, загруженные в редактор, сжимаются и уменьшаются.** Раньше картинка из CKEditor (форум, гостевая книга, комментарии новостей, форма статьи в админке) сохранялась как есть: без ограничения по весу, без ограничения по разрешению, а тип файла проверялся по расширению имени. Фотография с телефона на 10 МБ ложилась на диск в исходном разрешении.

  Теперь загрузка проходит через общий сценарий `Johncms\Image\StoreEditorImageUseCase`: тип определяется по содержимому файла (принимаются JPEG, PNG, GIF и WebP), слишком тяжёлый файл отклоняется с указанием лимита, а изображение больше заданных границ уменьшается с сохранением пропорций. Картинка, которая и так укладывается в границы, при формате хранения «исходный» сохраняется байт в байт — незачем терять качество там, где нечего выигрывать. Анимированные GIF не перекодируются, иначе от анимации остался бы один кадр.

  Настройки — в панели администратора, **Система → Изображения в редакторе** (`/admin/settings/images`):

  | Настройка | Значение по умолчанию | Что делает |
  |---|---|---|
  | Макс. размер изображения | 5120 КБ | предел на приём файла |
  | Макс. ширина / высота | 1600 × 1600 px | границы, до которых уменьшается картинка; `0` снимает ограничение по стороне |
  | Качество | 82 % | качество перекодирования |
  | Формат хранения | исходный | `исходный`, `JPEG` или `WebP` — два последних перекодируют каждую загрузку |

  Значения хранятся в `config/autoload/system.local.php` под ключом `johncms.editor_images`. На странице настроек показан фактический лимит PHP (`upload_max_filesize` и `post_max_size`) и предупреждение, если заданный лимит выше него — иначе загрузка сорвётся до того, как её увидит сайт.

  Попутно: тело запроса больше `post_max_size` PHP отбрасывает до запуска скрипта, и загрузчик получал в ответ «неверные данные» — теперь ему отвечают, что файл слишком велик для сервера. Общий лимит `johncms.flsz` («Макс. размер файла» в основных настройках) к редактору по-прежнему не относится: он про аватары, фото профиля, альбом и вложения форума.
