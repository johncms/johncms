  ...
  if (!$id) {
    ...
    if ($kol) {
      while ($row = mysql_fetch_assoc($sql)) {
        ...
        if ($adm) {
          echo '<div class="sub"><small>' . ($y != 1 ? '<a href="?act=move&amp;moveset=up&amp;posid=' . $y . '">Вверх</a> | ' : 'Вверх | ') . ($y != $kol ? '<a href="?act=move&amp;moveset=down&amp;posid=' . $y . '">Вниз</a>' : 'Вниз') . ' | <a href="?act=moder&amp;type=dir&amp;id=' . $row['id'] . '">' . $lng['edit'] . '</a> | <a href="?act=del&amp;type=dir&amp;id=' . $row['id'] . '">' . $lng['delete'] . '</a></small></div>';
        }
        echo '</div>';
      }
    }
    ...
    echo '<div class="phdr">Всего: ' . intval($kol) . '</div>';
    if ($adm) {
      ...
      if ($actdir) {
        ...
        if ($total) {
          ...
          while ($row = mysql_fetch_assoc($sql2)) {
            ...
            . ($obj->get_all_stat_tags() ? '<div>Теги [ ' . $obj->get_all_stat_tags(1) . ' ]</div>' : '')
