    ...
    switch($mode) {
        ...
        case 'delall':
            if (!isset($_GET['deldeny'])) {
                ...
                echo '<div class="gmenu">' . sprintf(_t('Successfully deleted:<br>Directories: (%d)<br>Articles: (%d)<br>Tags: (%d)<br>Comments: (%d)<br>Images: (%d)'), $deleted['dirs'], $deleted['texts'], $deleted['tags'], $deleted['comments'], $deleted['images']) . ')</div><div><a href="?">' . _t('Back') . '</a></div>' . PHP_EOL;
            }
        break;
