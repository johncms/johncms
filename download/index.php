...
function provcat($catalog)
{
    ...
    $cat1 = $db->query("SELECT * FROM `download` WHERE type = 'cat' AND id = '" . $catalog . "'");
    $cat2 = $cat1->rowCount();
    $adrdir = $cat1->fetch();
...
if (in_array($act, $array)) {
    ...
    if (empty($_GET['cat'])) {
        ...
        $req = $db->query("SELECT * FROM `download` WHERE `type` = 'cat' AND `id` = '" . $cat . "' LIMIT 1");
        $res = $req->fetch();

        if (!$req->rowCount() || !is_dir($res['adres'] . '/' . $res['name'])) {
    ...
    if ($total > 0) {
        $zap = $db->query("SELECT * FROM `download` WHERE `refid` = '$cat' ORDER BY `type` ASC, `text` ASC, `name` ASC LIMIT " . $start . "," . $kmess);

        while ($zap2 = $zap->fetch()) {
