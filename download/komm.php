...
$mess = $db->query("SELECT * FROM `download` WHERE type='komm' AND refid='" . $id . "' ORDER BY time DESC");
$countm = $mess->rowCount();
$fayl1 = $fayl->fetch();
...
while ($mass = $mess->fetch()) {
    if ($i >= $start && $i < $end) {
        ...
        $uz = $db->query("SELECT * FROM `users` WHERE name = " . $db->quote($mass['avtor']) . " LIMIT 1");
        $mass1 = $uz->fetch();
        echo "$div";
