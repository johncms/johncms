...
<head>
    ...
    @stack('styles')
    <title><?= (isset($title) ? e($title) : 'JohnCMS') ?></title>
</head>
<body>
    ...
    <div class="page_layout justify-content-end d-flex w-100">
        <div class="sidebar">
            <div class="sidebar__inner">
                ...
                <div class="sidebar__wrapper d-flex flex-column text-black-50 pb-3">
                    <?php foreach ($steps as $step): ?>
                        <div class="step d-flex align-items-center border-bottom px-2 py-3">
                            <div class="pe-3">
                                <svg class="icon">
                                    <use xlink:href="<?= asset('icons/sprite.svg') ?>#<?= $step['active'] ? 'check-circle' : 'circle' ?>"/>
                                </svg>
                            </div>
                            <div class="<?= $step['active'] || $step['current'] ? 'fw-bold' : '' ?> h5 mb-0"><?= $step['name'] ?></div>
                        </div>
                    <?php endforeach ?>
                </div>
            </div>
    ...
    <div class="page_layout w-100">
        <div class="content-container-padding footer-padding">
            <div class="d-flex justify-content-between align-items-center border-top p-4 page-footer">
                <div class="ps-1">
                    <a href="https://twitter.com/johncms" title="Twitter" target="_blank" rel="nofollow" class="me-4 text-muted text-decoration-none">
                        <svg class="icon">
                            <use xlink:href="<?= asset('icons/sprite.svg') ?>#twitter"/>
                        </svg>
                    </a>
                    <a href="https://t.me/johncms_official" title="Telegram" target="_blank" rel="nofollow" class="me-2 text-muted">
                        <svg class="icon">
