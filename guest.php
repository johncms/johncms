...
echo "<a href='guest.php?act=delpost&amp;id=".$id."&amp;yes'>Удалить</a>|<a href='guest.php'>Отмена</a><br/>";}}else{
...
$from=$login;$gs=0;}else{$from=$name;$gs=1;}
...
echo "Пост в гостевой:&quot;$ps1[name]: $ps1[text]&quot;<br/><br/><form action='guest.php?act=otvet&amp;id=".$id."' method='post'>Ответ:<br/><textarea rows='3' name='otv'>$ps1[otvet]</textarea><br/><input type='submit' name='submit' value='Ok!'/><br/></form><a href='guest.php?'>В гостевую</a><br/>";}}else{
echo "Доступ закрыт!!!<br/>";}
break;
...
echo "Редактировать пост:<br/><br/><form action='guest.php?act=edit&amp;id=".$id."' method='post'><textarea rows='3' name='msg'>$ps1[text]</textarea><br/><input type='submit' name='submit' value='Ok!'/><br/></form><a href='guest.php?'>В гостевую</a><br/>";}}else{
echo "Доступ закрыт!!!<br/>";}
break;
...
echo "Написать<br/><form action='guest.php?act=say' method='post'>";
if (empty($_SESSION['pid'])){
...
echo "<a href='anketa.php?user=".$mass1[id]."'><b><font color='".$conik."'>$mass[name]</font></b></a> ";}else{
echo "<b><font color='".$csnik."'>$mass[name]</font></b>";}}else{
...
$mass[text]=antilink($mass[text]);}else{
...
$mass[otvet]=eregi_replace("\\<a href\\='((https?|ftp)://)([[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/;%]*)?)?)'>[[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/;%]*)?)?)</a>", "<a href='\\1\\3'>\\3</a>" ,$mass[otvet]);}else{
...
$otvet=smilesadm($otvet);}else{$otvet=$mass[otvet];}
...
if ($start != 0) {echo '<a href="guest.php?page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$colmes && $asd>0){echo ' <a href="guest.php?page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="guest.php?page='.$paa.'">'.$paa.'</a> <a href="guest.php?page='.($paa+1).'">'.($paa+1).'</a> .. <a href="guest.php?page='.($paa*2).'">'.($paa*2).'</a> <a href="guest.php?page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+10;}
if ($page2>12){
echo ' .. <a href="guest.php?page='.$paa2.'">'.$paa2.'</a> <a href="guest.php?page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="guest.php?page='.($paa3).'">'.($paa3).'</a> <a href="guest.php?page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="guest.php?page='.$pa2.'">'.$pa2.'</a> <a href="guest.php?page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$colmes){echo ' .. <a href="guest.php?page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($colmes > $start + 10) {echo ' <a href="guest.php?page='.($page+1).'">&gt;&gt;</a>';}}

echo "<br/>Всего сообщений: $colmes<br/>";
break;}
