...
echo '' .
    ...
    '<div>' . functions::image('contacts.png') . '<a href="index.php">' . _t('Profile') . '</a></div>' .
    '<div>' . functions::image('rate.gif') . '<a href="?act=stat">' . _t('Statistics') . '</a></div>' .
    '<div>' . functions::image('photo.gif') . '<a href="../album/index.php?act=list">' . _t('Photo Album') . '</a>&#160;(' . $total_photo . ')</div>' .
...
echo '<div>' . functions::image('mail-inbox.png') . '<a href="../mail/index.php?act=input">' . _t('Received') . '</a>&nbsp;(' . $count_input . ($new_mail ? '/<span class="red">+' . $new_mail . '</span>' : '') . ')</div>';
...
echo '<div>' . functions::image('mail-send.png') . '<a href="../mail/index.php?act=output">' . _t('Sent') . '</a>&nbsp;(' . $count_output . ($count_output_new ? '/<span class="red">+' . $count_output_new . '</span>' : '') . ')</div>';
...
echo '<div>' . functions::image('mail-info.png') . '<a href="../mail/index.php?act=systems">' . _t('System') . '</a>&nbsp;(' . $count_systems . ($count_systems_new ? '/<span class="red">+' . $count_systems_new . '</span>' : '') . ')</div>';
...
echo '<div>' . functions::image('file.gif') . '<a href="../mail/index.php?act=files">' . _t('Files') . '</a>&nbsp;(' . $count_file . ')</div>';
...
echo '<div>' . functions::image('user.png') . '<a href="../mail/">' . _t('Contacts') . '</a>&nbsp;(' . $count_contacts . ')</div>';

//Заблокированные
$count_ignor = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='" . $user_id . "' AND `ban`='1'")->fetchColumn();
echo '<div>' . functions::image('user-block.png') . '<a href="../mail/index.php?act=ignor">' . _t('Blocked') . '</a>&nbsp;(' . $count_ignor . ')</div>';
echo '</p></div>';

// Блок настроек
echo '<div class="bmenu"><p><h3>' . _t('Settings') . '</h3>' .
    '<div>' . functions::image('user-edit.png') . '<a href="?act=edit">' . _t('Edit Profile') . '</a></div>' .
    '<div>' . functions::image('lock.png') . '<a href="?act=password">' . _t('Change Password') . '</a></div>' .
    '<div>' . functions::image('settings.png') . '<a href="?act=settings">' . _t('System Settings') . '</a></div>';
if ($rights >= 1) {
    echo '<div>' . functions::image('forbidden.png') . '<span class="red"><a href="../admin/"><b>' . _t('Admin Panel') . '</b></a></span></div>';
}
echo '</p></div>';
