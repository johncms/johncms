...
header ("Location: index.php?enter");exit;
}}
...
$regadmin=trim($reg1[regadm]); print ("<div style='text-align: center'>Ваша заявка на регистрацию одобрена администратором $regadmin</div>");}}

echo "Привет,$login !<br/>";
if ($dayr==$day&&$monthr==$mon){echo "<font color = 'red'><b>С ДНЁМ РОЖДЕНИЯ!!!</b></font><br/>";}
$birth = mysql_query("select * from `users` where preg='1' and dayb='".$day."' and monthb='".$mon."';");
$brd=mysql_num_rows($birth);
if ($brd>0){echo "Сегодня <a href='str/brd.php'>".$brd."</a> именинников<br/>";}



if (!isset($_GET['regprin'])&&!empty($datauser[lastdate])){echo "Последний раз вы заходили сюда $dpp с IP адреса $ipadr и браузера $soft<br/>";

echo "За это время новостей";
$nw = mysql_query("select * from `news` where
time>'".$datauser[lastdate]."';");
$kv=mysql_num_rows($nw);
if ($kv!=0){echo "<a href='str/news.php?kv=".$kv."'>: $kv</a><br/>";}else{echo " нет.<br/>";}
...
if ($knt!=0){$knt="Непрочитанных тем на форуме: <a href='forum/new.php'>$knt</a>";}
echo "$knt<br/>";

}else{echo "Добро пожаловать на $copyright !<br/>";}
echo "<a href='".$home."'>На главную</a></div></body></html>";exit; }


  
if (isset($_GET['err'])){print ("<div style='text-align: center'>Ошибка 404: файл не найден!!!</div>");}
if (isset($_GET['nolog'])){print ("<div style='text-align: center'>Ошибка:такой логин не зарегистрирован!!!</div>");}
if (isset($_GET['regwait'])){print ("<div style='text-align: center'>Сорри,ваша заявка ещё не рассмотрена,ожидайте</div>");}
