...
{
    echo "<div>Чё,самый умный,да????  ПШОЛНАХ ОТСЮДА!!!!</div>";
...
$pp = 0;
while ($dos11 = mysql_fetch_array($dos1))
{
    if ($pp < 1)
    {
        $idip = $dos11['id'];
...
$pp1 = 0;
while ($delid1 = mysql_fetch_array($delid))
{
    if ($pp1 < 1)
    {
        $iddel = $delid1['id'];
...
if ($bann == "1")
{
    ...
    if ($jjtti > 0)
    {
        ...
        {
            echo "<div>Причина:<font color='red'> $whybann</font></div>";
        }
