...
**5c. Изоляционный тест и приёмка (сам).** Тест в `tests/Functional/`, снятие оставшихся долгов,
запрет `di()` в новом коде правилом в `.agents/`.
...
**`title` и тема в этот критерий не входят** — они принадлежат `Render`, который чинится
отдельно (см. выше). Тест должен проверять именно то, что этап закрывает, иначе он останется
...
Между запросами протекут `title` и тема. Этап 5 это сознательно не чинит — детали и требования
