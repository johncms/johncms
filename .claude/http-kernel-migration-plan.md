...
**Остаток 5a (не сделано):** `User`/`System\Users\User` и `Translator` как per-request;
развязка `Environment` от Application-слоя шести модулей. Три оставшихся места захвата запроса
из контейнера (`FileStorage`, `forum/ForumUtils`, `TranslatorServiceFactory`) резолвят его
**на каждый вызов**, а `Request` синтетический и переиздаётся ядром — они уже получают
актуальный запрос, так что чинить там нечего до перевода `User`/`Translator`.
