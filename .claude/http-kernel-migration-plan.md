...
**5b. `Request` из конструктора в аргумент действия (сабагенты, коммит на модуль).** Та же форма,
что 2c: контекст локален, критерий приёмки объективный. Порядок по объёму: `forum` (35),
`admin` (21), `downloads` (20), `library` (12), `album` (11), `profile` (9), `guestbook` (7),
`contacts` (5), `mail` (4), `collections` (4), `consent` (3), далее по одному-два.
Плюс 8 не-контроллерных сервисов из пункта 3 — их разбирать вместе с их модулем.
