...
| `Render` (`extends Plates Engine`) | `addFolder()` + `addData()` (208 вызовов) — `title` предыдущего запроса утечёт в следующий. **Вынесен из этапа 5**: чинится вместе с переписыванием класса, см. ниже |
...
| `Environment` | добавлено 2026-07-24: синглтон, кэширует `$clientIp`, `$proxyIp`, `$userAgent`, `$ipCount` — чистое per-request состояние. Плюс `di(Request::class)` прямо в конструкторе (`Environment.php:51`) |
...
#### Ревизия 2026-07-28 (перед началом работ)

Всё сверено с кодом. Подтвердилось: `ActionInvoker` уже получает `Request` пайплайном (1e закрыт,
предпосылка для переноса в аргумент действия выполнена); `Request` уже `synthetic` (сделано в 3a);
`shared(false)` нет ни в одном `services.php` — все контроллеры синглтоны; `Environment` держит
`di(Request::class)` и `ipLog()` в конструкторе (`Environment.php:51`, номер строки уехал);
`Environment` в Application-слое тех же шести модулей; 208 вызовов `addData()`.

Уточнённые цифры: `Request` в конструкторе у **142 контроллеров из 222**, `$this->request` — **541
использование**. Из них **342 в самих действиях** (механическая правка) и **199 в приватных
хелперах 54 файлов** — там запрос придётся протаскивать параметром, это не sed. Ни один контроллер
не использует запрос в теле конструктора — то есть перенос ничего не ломает по порядку инициализации.

**Найдено сверх плана:**

1. **`initModule()` в конструкторах 216 файлов против per-request `Render`/`Translator`.**
   `ControllerContext::initModule()` регистрирует папку шаблонов модуля и домен переводов, и зовётся
   из конструктора контроллера. Контроллеры — синглтоны, значит регистрация происходит **один раз на
   процесс**. Если сделать `Translator` или `Render` per-request (пересоздавать или ресетить целиком),
   на втором запросе к тому же контроллеру не окажется ни домена переводов, ни папки шаблонов —
   отвалятся переводы и рендер. Поэтому `reset()` обязан быть точечным: у `Render` (наследник
   `Mobicms\Render\Engine`) чистится **только** `protected TemplateData $data`, а `$nameSpaces` и
   `$functions` сохраняются; `Translator` пересоздавать нельзя, у него сбрасывается только локаль.
   Альтернатива — вынести `initModule()` из конструкторов, но это правка тех же 216 файлов и
   отдельная задача.
2. **Глобалов больше, чем записано.** Кроме `$page`/`$start` в `system/bootstrap.php:95-96`:
   `$GLOBALS['old']` в трёх файлах (`downloads/IndexController.php:57`,
   `downloads/DownloadCategoryController.php:50`, читается в `downloads/FilePresenter.php:40`) и
   `global $mod, $start` в пяти (`system/src/Comments.php:125`, `downloads/FileCommentsController.php:120`,
   `album/PhotoCommentsController.php:57`, `profile/GuestbookController.php:51`,
   `library/ArticleCommentsController.php:76`).
3. **`Request` в конструкторе не только у контроллеров** — ещё 8 сервисов-синглтонов:
   `System/View/Theme`, `Http/Pagination/PaginationFactory`, `Http/Pagination/PaginationGuard`,
   `Http/Environment`, `guestbook/GuestbookForm`, `contacts/ContactForm`,
   `library/LegacyRedirectHandler`, `help/HelpLegacyRedirectHandler`.
4. **6 мест захватывают запрос из контейнера внутри сервиса**: `Environment.php:51`,
   `System/Users/UserFactory.php:49`, `Users/UserFactory.php:30`, `Files/FileStorage.php:35`,
   `forum/ForumUtils.php:64`, `System/i18n/TranslatorServiceFactory.php:26`. Первые пять — прямой
   захват в поле; фабрики читают запрос при сборке, что для synthetic-сервиса означает привязку
   к запросу момента сборки.
5. **Открытый вопрос 5 (storage сессии под воркером) остаётся открытым.** В 4b записано, что native
   storage не перезапускается после отправки заголовков, — это проверено на CLI и верно для
   RoadRunner-подобных бриджей. Под FrankenPHP worker mode контекст запроса сбрасывается на каждой
   итерации, так что native может и работать. Решать на этапе 6, замером, а не рассуждением.

#### Нарезка

**5a-1: ✅ ГОТОВО (2026-07-28, не закоммичено).** Глобалов в `modules/` и `system/` больше нет.
`$page`/`$start` из бутстрапа удалены (мертвы: каждый из четырёх вызывающих перетирал их своими);
резолв `System\Users\User` там оставлен — у него побочные эффекты (аутентификация по кукам,
проверка бана, история IP). `global $mod, $start` заменены на два ключа массива параметров,
который `Comments` и так принимает; `$start` кастуется внутри `Comments` (он идёт в `LIMIT`
сырой интерполяцией — раньше безопасность держалась на дисциплине четырёх вызывающих).
`$GLOBALS['old']` убран: `FilePresenter::NEW_FILE_PERIOD`.

**Побочно починен баг:** `FilePresenter` используют 10 контроллеров, а `$GLOBALS['old']` ставили
только два — в остальных восьми `?? 0` делал «новыми» все файлы. Теперь порог общий.

**Ограничение тестов:** `tests/Functional/Http/CommentsPageTest.php` проверяет только «не 5xx» и
полноту страницы, если стенд её отдал. Все четыре страницы `Comments` требуют предусловий
(файл на диске, пройденная премодерация, публичный альбом), а харнесс ходит гостем и фикстур
не создаёт. Полноценное покрытие нужно вместе с авторизацией в функциональных тестах.

**5a-2: ✅ ГОТОВО (2026-07-28, не закоммичено).** Введён `Symfony\...\RequestStack`: бутстрап кладёт
на него boot-запрос, ядро — запрос каждого цикла (`push` в начале `handle()`, `pop` в `finally`).
Сервисы, живущие дольше запроса, читают текущий запрос оттуда вместо захвата в поле.
`Environment` переведён на стек, `di(Request::class)` и `ipLog()` из конструктора убраны:
логирование частоты запросов стало `logRequestRate()` и зовётся ядром один раз за цикл (web-only).
`EnvironmentTest` больше не собирает объект через `newInstanceWithoutConstructor()`.
Появился механизм сброса: `instanceof(ResetInterface)` → тег `johncms.resettable` →
`tagged_iterator` в ядро, `reset()` на всех перед обработкой. Реализуют `Environment` и `NavChain`.
Проверено вручную: три запроса подряд в одном процессе дают по 2 крошки, а не 2/3/4.

**`Render` вынесен из этапа 5 (решение 2026-07-28).** Класс всё равно предстоит переписывать,
и чинить request-scope в нынешнем — работа, которую потом выбросят. Что выяснено попыткой и что
обязано учесть переписывание:

* Сброс одних только данных не работает: `RenderEngineFactory` кладёт
  `container`/`config`/`locale`/`user`/`csrf_token` через `addData()` **один раз при сборке**,
  шаблоны читают их как глобальные (`user.rights` в `modules/news/templates/public/index.phtml:14`).
  Чистка `TemplateData` уронила 26 тестов. Значит «глобалы» фабрики должны стать провайдером,
  применяемым заново на каждый запрос.
* `Render::$theme` — то же самое: админ-тема выбирается по URL запроса, а живёт как состояние
  процесса.
* Namespace'ы шаблонов регистрируются из конструкторов 216 контроллеров-синглтонов, то есть
  один раз на процесс — пересоздавать движок на запрос нельзя, пока это так.
* Провайдер глобалов резолвит `User` и `Translator`; их per-request делается здесь, в этапе 5,
  так что к моменту переписывания `Render` они уже будут готовы.

**Следствие: worker-режим до переписывания `Render` не заводится** — `title` и тема протекут
между запросами. Этап 6 этим и ограничен, см. его раздел.

**5a-3: ✅ ГОТОВО (2026-07-28, не закоммичено). `User` / `System\Users\User` per-request.**
Оба сервиса — синглтоны, которые сотни контроллеров и сервисов берут в конструктор, поэтому
пересоздавать их нельзя: заменяется **состояние**. `Johncms\Users\UserFactory::__invoke()` теперь
отдаёт гостя (стабильный экземпляр), а `authenticate(User, Request)` заливает в него посетителя
(`setRawAttributes` + `exists` + `setRelations([])` — связи прошлого посетителя обязаны уйти);
у `System\Users\User` то же делает новый `setProperties()`: объявленные свойства возвращаются
к дефолтам, динамические (колонки, которых класс не объявляет) удаляются. Координатор —
`Johncms\Users\CurrentUserAuthenticator`, идемпотентный по объекту запроса: под FPM бутстрап и
ядро обслуживают один и тот же `Request`, так что второй вызов не делает повторных запросов в БД.
Бутстрап зовёт его **до** резолва `Translator` (тот берёт язык из профиля) вместо прежнего
`$container->get(User::class)`; ядро — после старта сессии.

Побочно: `UserFactory` перешёл на `User::query()->find()` + `instanceof`, из baseline ушли 7 записей.

**Найдена и починена утечка процесса: константа `_IS_HOMEPAGE`.** `HomepageController` объявлял
её через `define()`, а `Ads::checkAccess()` и `Counters::counters()` читали `defined()`. Под
воркером первый заход на главную делал главной **каждую** следующую страницу (реклама с
`layout = 1`, счётчики `link1`). Теперь это атрибут запроса (`is_homepage`), контроллер получает
`Request` аргументом действия, `Ads`/`Counters` читают его через `RequestStack`. Нашёл это
функциональный тест: второй `handle()` главной в одном процессе дал `Constant already defined`.

**5a-4: ✅ ГОТОВО (2026-07-28, не закоммичено). `Translator` per-request.** Логика выбора языка
вынесена из `TranslatorServiceFactory` в `Johncms\System\i18n\LocaleResolver` (`?setlng` → сессия
→ профиль → системный дефолт); ядро после аутентификации зовёт
`$translator->setLocale($localeResolver->resolve())`.

**Ключевое:** одной смены локали мало. `addTranslationDomain()` грузит `<locale>.lng.php` в момент
регистрации, а домены регистрируются из конструкторов контроллеров-синглтонов, то есть **один раз
на процесс**. Поэтому `Translator::setLocale()` теперь перезагружает словарь: класс запоминает
зарегистрированные домены и при смене локали чистит `dictionary`/`plurals` и грузит их заново,
...
**5a-7: ✅ ГОТОВО (2026-07-29, не закоммичено). `Session` per-request.** Фасад реализует
`ResetInterface`: `reset()` пересоздаёт внутренний `Symfony\...\Session` поверх того же storage,
то есть выбрасывает бэги обслуженного запроса. Открытую сессию `reset()` не трогает — под FPM её
открывает бутстрап **до** ядра, и сброс бэгов потерял бы записанное текущим запросом.

Чтобы «открыта» всегда значило «принадлежит текущему запросу», `session->save()` в ядре переехал
**в `finally`**: цикл, упавший до сохранения, больше не оставляет сессию открытой на следующий
(в воркере это отдало бы посетителю сессию того, кто упал перед ним).

Storage под воркером остаётся открытым вопросом 5 (native не переоткрывается после отправки
заголовков) — тест `SessionTest::testTheSessionThatCannotBeReopened…` фиксирует, что в этом случае
фасад падает, а не отдаёт данные прошлого посетителя.

**Побочно: консольное приложение больше не собирается на каждый HTTP-запрос.**
`Symfony\...\Console\Application` реализует `ResetInterface`, поэтому правило
`instanceof(ResetInterface)` из 5a-2 вешало на него `johncms.resettable`, а `tagged_iterator`
в ядре инстанцирует всё, что помечено, — то есть каждый веб-запрос строил CLI-приложение со
всеми командами. Определение сервиса перенесено **выше** правила `instanceof` (условия
применяются только к последующим определениям; `autoconfigure(false)` тут не работает).
Сторожит `ContainerCompilationTest::testTheResettableServicesAreTheRequestScopedOnesOnly`.

**Остаток 5a:** пусто — инфраструктура request-scope закрыта. Три места захвата запроса из контейнера (`FileStorage`, `forum/ForumUtils`,
...
**5a. Инфраструктура request-scope (сам, один коммит).** ✅ Закрыт целиком (5a-1 … 5a-7), кроме
`Render` — он сознательно вынесен из этапа (см. выше). Исходный объём:
Точечный `reset()` там, где он нужен
(`Render` — только `TemplateData`; `NavChain` — `$items`; `Environment`; `Session`), вызов из
`Kernel::handle()`; оставшиеся per-request сервисы — `->synthetic()` с публикацией из ядра;
развязка `Environment` (убрать `di(Request)` и `ipLog()` из конструктора, после чего снять
`newInstanceWithoutConstructor()` в `EnvironmentTest`) и его вынос из Application-слоя шести
модулей; 6 мест захвата запроса из контейнера; глобалы `$page`/`$start`, `$GLOBALS['old']`,
`global $mod, $start`.

**5b. ✅ ГОТОВО (2026-07-29). `Request` из конструктора в аргумент действия.** Ни одного
контроллера с `Request` в конструкторе и ни одного `$this->request` в контроллерах не осталось
ни в `modules/`, ни в `system/src/`. Прошло тремя батчами сабагентов (`forum`; `admin` +
`downloads`; `library` + `album` + `profile` + `guestbook`; хвост + сервисы), коммит на модуль.
Форма та же, что 2c: контекст локален, критерий приёмки объективный.

Выработанные правила, единые по всей кодовой базе:

* `Request $request` — **первым** аргументом действия, перед параметрами маршрута. Для
  `ActionInvoker` порядок безразличен (классовые аргументы резолвятся по типу, параметры маршрута
  по имени), но единообразие важнее.
* Запрос добавляется **только** в действия, которые его читают: `form`/`confirm`/`*Form`
  сигнатуры сохранили.
* Приватным хелперам запрос передаётся **параметром**, а не остаётся полем.
* Делегирующие контроллеры (`ForumPathController`, `DownloadPathController`, `HelpIndexController`)
  вызывают `__invoke()` соседа напрямую и ломаются от смены его сигнатуры, при этом сами `Request`
  в конструкторе могли не держать — искать грепом по `->__invoke(`, в первичный список они
  не попадают.

**Восемь не-контроллерных сервисов пункта 3 разошлись по двум решениям.** Где сервису нужен набор
полей тела — запрос уходит параметром метода (`guestbook/GuestbookForm::getFormData()`,
`contacts/ContactForm::getFormData()`, `library/LegacyRedirectHandler::handle()`,
`help/HelpLegacyRedirectHandler::handle()`); `forum/ForumUtils::topicLink()` обошёлся одним фактом
(`string $host`) по линии 5a-6. Где сервис вызывается из десятков мест, чьи действия запрос не
берут вовсе, — `RequestStack`: `PaginationFactory` и `PaginationGuard` (259 упоминаний в ~70
файлах), `Theme` (резолвится прямо из шаблонов через `di()`, запроса там физически нет).
`Environment` правки не потребовал — на стеке с 5a-2. Граница правила: `RequestStack` допустим
в `system/src` для синглтонов, переживающих запрос, но не в Application-слое модулей — там он
воспроизводит ровно тот скрытый захват, который этап убирает.

**Шесть живых багов из-под записей baseline** (урок 2c подтвердился в третий и последующие разы —
записи baseline прячут дефекты, а не только шум):

* `downloads/ImportFileController::handleImport()` читал `$category` из области видимости
  вызывающего — успешный импорт падал `TypeError` в `getCategoryUrl()`.
* `library/CreateArticleController::renderForm()` вызывался с 9 аргументами из 10 на обеих ветках
  ошибок валидации — флуд-контроль или пустой заголовок давали `ArgumentCountError` вместо формы
  с сообщением.
* `contacts/ContactSettingsController::postedTranslations()` и
  `consent/CookieBannerController::buildDto()` читали мультиязычные поля-массивы через `body()`,
  который отдаёт строку: `InputBag` бросал `BadRequestException`, и сохранение обеих страниц
  настроек отвечало 400 при любом заполненном переводе. Переведены на `bodyList()`.
* `mail/BlocklistIndexController` держал `Request`, который никогда не читался.

Побочно: `phpstan-baseline.neon` за этап только уменьшился, ни одного нового подавления.

**5c. ✅ ГОТОВО (2026-07-29). Изоляционный тест и приёмка.**
`tests/Functional/Http/RequestIsolationTest.php` — семь проверок: один и тот же запрос дважды даёт
тот же ответ; запрос между двумя одинаковыми не меняет ответ следующего (утечка синглтон-контроллера
видна только после другого маршрута); один контроллер на двух разных query отвечает своим (5b);
пагинация следует странице текущего запроса (`RequestStack`); крошки не накапливаются; факты
посетителя не переживают цикл; контейнер публикует запрос цикла, а стек возвращается к boot-записи.

Правило «как класс получает текущий запрос» записано в `.agents/architecture.md` (аргумент действия
→ параметр хелпера → факт → `Request` параметром метода → `RequestStack` только в `system/src`),
там же запрет резолвить запрос из контейнера. Последний долг снят: `FileStorage::saveFromRequest()`
принимает `Request` параметром вместо `di(Request::class)` (четыре вызывающих, у всех запрос уже
в области видимости действия), так что в `modules/` и `system/src/` запрос из контейнера не
резолвит никто.

**Уточнение 2026-07-30:** резолвы, остававшиеся за пределами этих двух каталогов — `public/install/`
(5 файлов, из-за них падал инсталлятор), `themes/default/.../layout/default.phtml`, мёртвая ветка
`di('route')` в `system/helpers.php` и `public/index.php`, — сняты вместе с самим синтетическим
сервисом. Запрос больше не сервис контейнера: `di(Request::class)` бросает `ServiceNotFound`.

Устаревшие ограничения набора убраны: докблок `FunctionalTestCase` больше не обещает «один запрос
на маршрут на процесс», `/community/search` вернулся в смоук — он был исключён именно из-за
залипавшего запроса.

**Тест поймал долг `Render` и уточнил его механизм.** Первая редакция сравнивала ответы целиком
и падала: `/forum` между двумя `/help` менял у второго `canonical` и обнулял `keywords`/`description`.
Причина — `addData()` **без списка шаблонов** (`HomepageController`, `ForumIndexController` и ещё
206 вызовов): в mobicms/render это `shareWithAll()`, то есть мета одной страницы живёт в движке
до конца процесса. Протекает не только `title` и тема, как записано выше, а вся head-мета.
**Следствие для переписывания `Render`**: провайдера «глобалов процесса» недостаточно — нужно
разделить глобалы (`user`, `locale`, `csrf_token`) и мету страницы, чьё время жизни равно запросу.
Тест поэтому сравнивает `<body>`, а не весь ответ.

**Готово, когда**: два последовательных `handle()` в одном процессе не протекают ничем, что
входит в scope этапа — крошки `NavChain`, состояние `Environment`, текущий запрос, сессия,
`Request` в контроллерах. Тест на изоляцию: один и тот же запрос дважды и сравнение ответов,
затем два разных запроса. Инфраструктура для него появилась в 3a вместе с `tests/Functional/`.
**`title`, тема и остальная head-мета в этот критерий не входят** — они принадлежат `Render`,
который чинится отдельно (см. выше и разбор в 5c). Тест должен проверять именно то, что этап закрывает, иначе он останется
красным по причине, до которой этап не дотягивается.
**Важно для 5b**: пока 5a не сделан, перенос `Request` в аргумент действия сам по себе изоляции
не даёт — контроллер-синглтон перестаёт держать запрос, но `Render`/`NavChain`/`Environment`
продолжают течь. Порядок 5a → 5b обязателен.

### — Этап 6. Рантайм (S)

**Блокер (зафиксирован 2026-07-28): пока `Render` не переписан, worker-режим не заводится.**
Движок — синглтон, чьи «глобалы» (`user`, `locale`, `csrf_token`) и тема ставятся один раз при
сборке, а namespace'ы шаблонов регистрируются из конструкторов 216 контроллеров-синглтонов.
Между запросами протекут тема и вся head-мета страницы (`title`, `canonical`, `keywords`,
`description` — механизм разобран в 5c). Этап 5 это сознательно не чинит — детали и требования
к переписыванию в его разделе. Поэтому этап 6 стартует только после переписывания `Render`,
либо ограничивается подготовкой (бридж, Docker-профиль) без объявления режима рабочим.
