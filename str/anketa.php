...
if($arr2==0){echo "Пользователя с таким id не существует!<br/>";require ("../incfiles/end.php");exit;}
...
switch ($act){
case "name":
echo "<form action='anketa.php?user=".$idus."&amp;act=editname' method='post'>Изменить имя(max. 15):<br/><input type='text' name='nname' value='".$arr[imname]."'/><br/><input type='submit'  value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
case "par":
echo "<form action='anketa.php?user=".$idus."&amp;act=editpar' method='post'>Старый пароль:<br/><input type='text' name='par1'/><br/>Новый пароль:<br/><input type='text' name='par2'/><br/>Подтвердите пароль:<br/><input type='text' name='par3'/><br/><input type='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
$par22=md5(md5($par2));
if ($par11!==$passw){echo"Неверно указан текущий пароль<br/><a href='anketa.php?act=par&amp;user=".$idus."'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
if ($par2!==$par3){echo"Вы ошиблись при подтверждении нового пароля<br/><a href='anketa.php?act=par&amp;user=".$idus."'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
if ($par2==""){echo"Вы не ввели новый пароль<br/><a href='anketa.php?act=par&amp;user=".$idus."'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
...
case "gor":
echo "<form action='anketa.php?user=".$idus."&amp;act=editgor' method='post'>Изменить город(max. 20):<br/><input type='text' name='ngor' value='".$arr[live]."'/><br/><input type='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
case "inf":
echo "<form action='anketa.php?user=".$idus."&amp;act=editinf' method='post'>Изменить инфу(max. 500):<br/><input type='text' name='ninf' value='".$arr[about]."'/><br/><input type='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
case "icq":
echo "<form action='anketa.php?user=".$idus."&amp;act=editicq' method='post'>Изменить ICQ:<br/><input type='text' name='nicq' value='".$arr[icq]."'/><br/><input type='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
case "mobila":
echo "<form action='anketa.php?user=".$idus."&amp;act=editmobila' method='post'>Изменить модель телефона(max.50):<br/><input type='text' name='nmobila' value='".$arr[mibile]."' /><br/><input type='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
$i=1;	while ($i<=31){ echo "<option value='".$i."'>$i</option>";++$i;}
$mnt=$arr[monthb];
echo "</select>
<select name='user_month' class='textbox'><option value='".$mnt."'>$mesyac[$mnt]</option>";
$i=1;	while ($i<=12){ echo "<option value='".$i."'>$mesyac[$i]</option>";++$i;}
echo "</select>
<select name='user_year' class='textbox'><option>$arr[yearofbirth]</option>";
$i=1950;while ($i<=2000){ echo "<option value='".$i."'>$i</option>";++$i;}
...
case "site":
echo "<form action='anketa.php?user=".$idus."&amp;act=editsite' method='post'>Изменить сайт(max. 50):<br/><input type='text' name='nsite' value='".$arr[www]."'/><br/><input type='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";
break;
...
if ($nmail!=$arr[mail]){$nmailact=0;}
else{$nmailact=$arr[mailact];}
...
echo "<a href='anketa.php?user=".$idus."'>В анкету</a><br/>";}else{
echo "Неверный код<br/>";
echo "<a href='anketa.php?act=activmail&amp;user=".$idus."&amp;continue'>Повторить</a><br/>";}}
else{
echo "<form action='anketa.php?user=".$idus."&amp;act=activmail&amp;continue' method='post'>Код активации:<br/><input type='text' name='provact'/><br/><input type='submit' name='submit' value='ok'/></form><br/><a href='anketa.php?user=".$idus."'>Назад</a><br/>";}
require ("../incfiles/end.php");exit;}
if ($_SESSION['activ']!=1){
...
$_SESSION['activ']=1;}else{echo "Код для активации уже выслан<br/>";}
...
if ($arr[mailact]==0){echo "<font color='".$clink."'>(!)</font>";}
...
$whoban=$arr[who];$whyban=$arr[why];
if($arr[ban]=="1"&&$arr[bantime]>$realtime||$arr[ban]=="2"){
echo"Юзер забанен<br/>";
echo"Забанил <font color='".$clink."'>$whoban</font><br/>";	
if($arr[why]==""){echo"Причина не указана<br/>";}else{echo"Причина:<font color='".$clink."'> $whyban</font><br/>";}
if($arr[ban]=="1"){
...
if($nb[why]==""){echo"Причина не указана<br/>";}else{echo"Причина:<font color='".$clink."'> $nb[why]</font><br/>";}
...
echo "$stats<br/>";}
 if ($dostadm=="1"){
if ($preg==0&&$regadm==""){echo "Ожидает подтверждения регистрации<br/>";}
if ($preg==0&&$regadm!=""){echo "Регистрацию отклонил $regadm<br/>";}
if ($preg==1&&$regadm!=""){echo "Регистрацию подтвердил $regadm<br/>";}
if ($preg==1&&$regadm==""){echo "Регистрация без подтверждения<br/>";}
}
if($arr[ban]=="1"&&$arr[bantime]>$realtime||$arr[ban]=="2"){
...
if ($arr[sex]=="zh")
{echo "Девушка<br/>";}
if (!empty($arr[dateofbirth])){echo "Дата рождения: $arr[dayb] $mesyac[$mmon] $arr[yearofbirth]<br/>";}
if ($arr['sex']=="m")
{
...
echo "E-mail:";
if ($arr[mailvis]==1){
echo "$arr[mail]<br/>";}else{
echo "скрыт<br/>";}}}
if (!empty($arr[icq])){echo '<img src="http://web.icq.com/whitepages/online?icq='.$arr[icq].'&amp;img=5" alt=""/> ICQ:'.$arr[icq].' <br/> ';}

if (!empty($arr[www]) && $arr[www]!=="http://" && stristr($arr[www],"http://")){
$sait=str_replace("http://","",$arr[www]);
echo "Сайт: <a href='".$arr[www]."'>$sait</a><br/>";}
if (!empty($arr[about])){echo "О себе: $arr[about]<br/>";}
if (!empty($arr[live])){echo "Город: $arr[live]<br/>";}
if (!empty($arr[mibile])){echo "Мобила: $arr[mibile]<br/>";}
...
$ivr=$svr1-$mvr*60;
if ($ivr=="60"){$ivr="59";}
$sitevr="$hvr ч. $mvr мин. $ivr сек. ";}else{ if ($svr>="60"){$mvr=ceil($svr/60)-1;
$ivr=$svr-$mvr*60;
if ($ivr=="60"){$ivr="59";}$sitevr="$mvr мин. $ivr сек. ";}else{$ivr=$svr;$sitevr="$ivr сек. ";}}
if ($arr['sex']=="m"){
echo "Провел ";}
if ($arr['sex']=="zh")
{echo "Провела ";}
...
if ($conts!=1){echo "<a href='cont.php?act=edit&amp;id=".$user."&amp;add=1'>Добавить в контакты</a><br/>";
  }else{echo "<a href='cont.php?act=edit&amp;id=".$user."'>Удалить из контактов</a><br/>";}
...
echo "<a href='ignor.php?act=edit&amp;id=".$user."&amp;add=1'>Добавить в игнор</a><br/>";}
  }else{echo "<a href='ignor.php?act=edit&amp;id=".$user."'>Удалить из игнора</a><br/>";
  }
...
}else{echo "Вы не авторизованы!<br/>";}
