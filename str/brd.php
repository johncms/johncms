...
if ($start != 0) {echo '<a href="brd.php?page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$count && $asd>0){echo ' <a href="brd.php?page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="pbrd.php?page='.$paa.'">'.$paa.'</a> <a href="brd.php?page='.($paa+1).'">'.($paa+1).'</a> .. <a href="brd.php?page='.($paa*2).'">'.($paa*2).'</a> <a href="brd.php?page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+10;}
if ($page2>12){
echo ' .. <a href="brd.php?page='.$paa2.'">'.$paa2.'</a> <a href="brd.php?page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="brd.php?page='.($paa3).'">'.($paa3).'</a> <a href="brd.php?page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="brd.php?page='.$pa2.'">'.$pa2.'</a> <a href="brd.php?page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$count){echo ' .. <a href="brd.php?page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($count > $start + 10) {echo ' <a href="brd.php?page='.($page+1).'">&gt;&gt;</a>';}
