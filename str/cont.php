...
echo "Контакт добавлен<br/>";}else{echo "Данный логин отсутствует в базе данных<br/>";}}else{echo "Данный логин уже есть в Ваших контактах<br/>";}}else{  
if  ($adc1==1){
if  ($addc1==1){

mysql_query("delete from `privat` where me='".$login."' and cont='".$nik."';");
echo "Контакт удалён<br/>";}else{echo "Данный логин отсутствует в базе данных<br/>";}}else{echo "Этого логина нет в Ваших контактах<br/>";}}
...
if ($userr[mailact]==1){
if (!empty($userr[icq])){echo '<img src="http://web.icq.com/whitepages/online?icq='.$userr[icq].'&amp;img=5" alt=""/> ICQ:'.$userr[icq].' <br/> ';}
if (!empty($userr['mail'])){
echo "E-mail:";
if ($userr[mailvis]==1){
echo "$userr[mail]<br/>";}else{
echo "скрыт<br/>";}}}
if (!empty($userr[www]) && $userr[www]!=="http://" && stristr($userr[www],"http://")){echo "Сайт: <a href='".$userr['www']."'>".$userr['www']."</a><br/>";}
...
<a href='op:fileselect'>Выбрать файл</a><hr/>
  ...
  echo "<a href='cont.php?act=trans'>Транслит</a><br /><a href='smile.php'>Смайлы</a><br /><a href='?'>В список</a></div>";  
	}else{echo "Ошибка-не указан адресат<br/>";}
break;
