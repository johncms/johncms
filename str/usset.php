...
"#FF0066"=>"Розовый",
"#FF6666"=>"Коралл",
"#FF9999"=>"Светлый коралл",
"#663300"=>"Тёмно-коричневый",
"#666600"=>"Коричневый",
"#CC6600"=>"Светло-коричневый",
"#996600"=>"Горчичный",
"#333333"=>"Тёмно-серый",
"#666666"=>"Серый",
"#999999"=>"Светло-серый",
...
if ($i>=0&&$i<4||$i>6&&$i<13||$i==17||$i==18||$i>21&&$i<=33){$numb[]=$color; }}
switch ($act){
######
case "help":
...
if(in_array($k,$numb)){$cl="#FFFFFF";}else{$cl="#000000";}
...
mysql_query("update `users` set bgcolor='".$acl."',bclass='".$bcl."',cclass='".$ccl."',link='".$lcl."',tex='".$tcol."',pfon='".$pfon."',cpfon='".$cpfon."',ccfon='".$ccfon."',cctx='".$cctx."',cntem='".$cntem."',ccolp='".$ccolp."',cdtim='".$cdtim."',cssip='".$cssip."',csnik='".$csnik."', conik='".$conik."',cadms='".$cadms."',cons='".$cons."',coffs='".$coffs."',cdinf='".$cdinf."'  where id='".intval($_SESSION['pid'])."';"); 
header ("Location: usset.php?act=color&yes");}else{
require("../incfiles/head.php");
require("../incfiles/inc.php");
if (isset($_GET['yes'])){echo "Ваши цветовые настройки изменены!<br/>";}
...
echo"Отделить фоном ник,дату и т.д. от текста поста:<br/>Да&nbsp;&nbsp;&nbsp;&nbsp;";
	if($datauser[pfon]=="1"){echo"<input name='pfon' type='radio' value='1' checked='checked'/>";} else {echo"<input name='pfon' type='radio' value='1' />";} 	echo" &nbsp; &nbsp; ";
	if($datauser[pfon]=="0"){echo"<input name='pfon' type='radio' value='0' checked='checked' />";} else {echo"<input name='pfon' type='radio' value='0'/>";}
	echo"Нет<br/>";
echo "Цвет фона<br/><select name='acl'><br/>"; 
if (!empty($fon)){
echo "<option value= '".$fon."'>$arrcol[$fon]</option>";}
else{echo "<option value= '#666666'>Серый</option>";}
...
else{echo "<option value= '#009999'>Бирюзовый</option>";}
foreach ($arrcol as $key=>$val){
...
else{echo "<option value= '#0000FF'>Светло-синий</option>";}
foreach ($arrcol as $key=>$val){
...
else{echo "<option value= '#000000'>Чёрный</option>";}
foreach ($arrcol as $key=>$val){
...
else{echo "<option value= '#CCCCCC'>Серебряный</option>";}
foreach ($arrcol as $key=>$val){
...
else{echo "<option value= '".$clink."'>$arrcol[$clink]</option>";}
foreach ($arrcol as $key=>$val){
...
else{echo "<option value= '".$fon."'>$arrcol[$fon]</option>";}
foreach ($arrcol as $key=>$val){
...
else{echo "<option value= '".$colt."'>$arrcol[$colt]</option>";}
foreach ($arrcol as $key=>$val){
...
if ($dostsadm==1){
if (!empty($_POST['snas'])){
$nastr=check(trim($_POST['snas']));}else{
$nastr=check(trim($_POST['nastr']));}}else{
...
if ($chmess<5){$chmess=5;}
if ($chmess>30){$chmess=30;}
...
require("../incfiles/head.php");
require("../incfiles/inc.php");
if (isset($_GET['yes'])){echo "Ваши настройки чата изменены!<br/>";}
$nastr=array("без настроения","бодрое","прекрасное","весёлое","Унылое","ангельское","агрессивное","изумлённое","удивленное","злое","сердитое","сонное","озлобленное","скучающее","оживлённое","угрюмое","размышляющее","занятое","нахальное","холодное","смущённое","крутое","смутённое","дьявольское","сварливое","счастливое","горячее","влюблённое","невинное","вдохновлённое","одинокое","скрытое","пушистое","задумчивое","психоделическое","расслабленое","грустное","испуганное","шокированное","потрясенное","больное","хитрое","усталое","утомленное");
echo "<form action='usset.php?act=chat' method='post' >Настройки чата<br/>Время обновления в чате<br/><input type='text' name='refresh' value='".$datauser[timererfesh]."'/><br/>Количество постов на страницу:<br/><input type='text' name='chmess' value='".$datauser[chmes]."'/><br/>";
echo"Поле ввода:<br/>Вкл.&nbsp;&nbsp;";
	if($datauser[carea]=="1"){echo"<input name='charea' type='radio' value='1' checked='checked'/>";} else {echo"<input name='charea' type='radio' value='1' />";} 	echo" &nbsp; &nbsp; ";
	if($datauser[carea]=="0"){echo"<input name='charea' type='radio' value='0' checked='checked' />";} else {echo"<input name='charea' type='radio' value='0'/>";}
	echo"Выкл.<br/>";
echo "Настроение:<br/><select name='nastr'>";
if (!empty($nastroy)){echo "<option>$nastroy</option>";}
foreach ($nastr as $v){
if ($v!=$nastroy){
...
$nastroy1="";}else{$nastroy1=$nastroy;}
...
if ($kolmess<5){$kolmess=5;}
if ($kolmess>30){$kolmess=30;}
...
if (isset($_GET['yes'])){echo "Ваши настройки форума изменены!<br/>";}
...
echo"Новые посты:<br/>Внизу";
	if($datauser[upfp]=="0"){echo"<input name='upfp' type='radio' value='0' checked='checked'/>";} else {echo"<input name='upfp' type='radio' value='0' />";} 	echo" &nbsp; &nbsp; ";
	if($datauser[upfp]=="1"){echo"<input name='upfp' type='radio' value='1' checked='checked' />";} else {echo"<input name='upfp' type='radio' value='1'/>";}
	echo"Вверху<br/>";
echo"Поле ввода:<br/>Вкл.&nbsp;&nbsp;";
	if($datauser[farea]=="1"){echo"<input name='farea' type='radio' value='1' checked='checked'/>";} else {echo"<input name='farea' type='radio' value='1' />";} 	echo" &nbsp; &nbsp; ";
	if($datauser[farea]=="0"){echo"<input name='farea' type='radio' value='0' checked='checked' />";} else {echo"<input name='farea' type='radio' value='0'/>";}
	echo"Выкл.<br/>";
...
echo "<input type='checkbox' name='nmenu[]' value='".$k."' checked='checked'/>$v<br/>";}else{
...
require("../incfiles/head.php");
require("../incfiles/inc.php");
echo "Ошибка!<br/>";require("../incfiles/inc.php");exit;}
$id=intval(check($_GET['id']));
$th = mysql_query("select * from `themes` where id='".$id."';");
$thm=mysql_fetch_array($th);
if (isset($_POST['submit'])){
mysql_query("update `users` set pfon='".$thm[pfon]."',cpfon='".$thm[cpfon]."',ccfon='".$thm[ccfon]."',cctx='".$thm[cctx]."',bgcolor='".$thm[bgcolor]."',tex='".$thm[tex]."',link='".$thm[link]."',bclass='".$thm[bclass]."',cntem='".$thm[cntem]."',ccolp='".$thm[ccolp]."',cdtim='".$thm[cdtim]."',cssip='".$thm[cssip]."',csnik='".$thm[csnik]."',cclass='".$thm[cclass]."',conik='".$thm[conik]."',cadms='".$thm[cadms]."',cons='".$thm[cons]."',coffs='".$thm[coffs]."',cdinf='".$thm[cdinf]."' where id='".intval(check($_SESSION['pid']))."';"); 
header ("Location: usset.php?act=color&yes");}else{
require("../incfiles/head.php");
require("../incfiles/inc.php");
...
echo "<div style='background:".$thm[bclass].";'><img src='../images/op.gif' alt=''/><font color='".$thm[cntem]."'>Название темы</font><font color='".$thm[ccolp]."'> [Кол-во постов]</font><br/><font color='".$thm[cdtim]."'>(Дата и время)</font><br/><font color='".$thm[cssip]."'>[Автор темы и посл. поста]</font></div><div style='background:".$thm[cclass].";'><img src='../images/np.gif' alt=''/><font color='".$thm[cntem]."'>Название темы</font><font color='".$thm[ccolp]."'> [Кол-во постов]</font><br/><font color='".$thm[cdtim]."'>(Дата и время)</font><br/><font color='".$thm[cssip]."'>[Автор темы и посл. поста]</font></div><hr/>";
...
if ($thm[pfon]==1){echo "<div style='background:".$thm[cpfon].";'>";}

echo "<img src='../images/m.gif' alt=''/><b><font color='".$thm[csnik]."'>Свой ник</font></b><font color='".$thm[cadms]."'> Адмстатус </font><font color='".$thm[cons]."'> [ON]</font><font color='".$thm[cdtim]."'>(Время поста)</font><br/>";
if ($thm[pfon]==1){echo "</div>";}

echo "<font color='".$thm[tex]."'>Текст</font><br/><font color='".$thm[cdinf]."'>Доп. информация</font><br/></div><div style='background:".$thm[cclass].";'>";

if ($thm[pfon]==1){echo "<div style='background:".$thm[cpfon].";'>";}

echo "<img src='../images/f.gif' alt=''/><b><font color='".$thm[conik]."'>Чужой ник</font></b> <font color='".$thm[conik]."'> [ц]</font><font color='".$thm[coffs]."'> [Off]</font><font color='".$thm[cdtim]."'>(Время поста)</font><br/>";
if ($thm[pfon]==1){echo "</div>";}
...
if ($start != 0) {echo '<a href="usset.php?act=other&amp;page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$count && $asd>0){echo ' <a href="usset.php?act=other&amp;page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="usset.php?act=other&amp;page='.$paa.'">'.$paa.'</a> <a href="usset.php?act=other&amp;page='.($paa+1).'">'.($paa+1).'</a> .. <a href="usset.php?act=other&amp;page='.($paa*2).'">'.($paa*2).'</a> <a href="users.php?page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+10;}
if ($page2>12){
echo ' .. <a href="usset.php?act=other&amp;page='.$paa2.'">'.$paa2.'</a> <a href="usset.php?act=other&amp;page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="usset.php?act=other&amp;page='.($paa3).'">'.($paa3).'</a> <a href="usset.php?act=other&amp;page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="usset.php?act=other&amp;page='.$pa2.'">'.$pa2.'</a> <a href="usset.php?act=other&amp;page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$count){echo ' .. <a href="usset.php?act=other&amp;page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($count > $start + 10) {echo ' <a href="usset.php?act=other&amp;page='.($page+1).'">&gt;&gt;</a>';}
...
require("../incfiles/inc.php");
if (isset($_GET['yes'])){
mysql_query("update `themes` set pfon='".$pfon."',cpfon='".$cpfon."',ccfon='".$ccfon."',cctx='".$cctx."',bgcolor='".$fon."',tex='".$colt."',link='".$clink."',bclass='".$clb."',cntem='".$cntem."',ccolp='".$ccolp."',cdtim='".$cdtim."',cssip='".$cssip."',csnik='".$csnik."',cclass='".$clc."',time='".$realtime."',conik='".$conik."',cadms='".$cadms."',cons='".$cons."',coffs='".$coffs."',cdinf='".$cdinf."' where name='".$login."';"); 
echo "Ваша цветовая схема перезаписана!<br/><a href='usset.php'>В настройки</a><br/>";}else{
$q1 = mysql_query("select * from `themes` where name='".$login."';");
$q2=mysql_num_rows($q1);
if ($q2==1){echo " У Вас уже есть сохраненная цветовая схема.<br/>Перезаписать?<br/><a href='usset.php?act=save&amp;yes'>Да</a> | <a href='usset.php?act=forum'>Нет</a><br/>";require ("../incfiles/end.php");exit;}
mysql_query("insert into `themes` values(0,'".$login."','".$realtime."','".$fon."','".$colt."','".$clink."','".$clb."','".$clc."','".$pfon."','".$cpfon."','".$ccfon."','".$cctx."','".$cntem."','".$ccolp."','".$cdtim."','".$cssip."','".$csnik."','".$conik."','".$cadms."','".$cons."','".$coffs."','".$cdinf."');");
...
if (isset($_GET['yes'])){echo "Ваши настройки изменены!<br/>";}
...
echo"Графика:<br/>Вкл.";
	if($offgr=="0"){echo"<input name='offgr' type='radio' value='0' checked='checked'/>";} else {echo"<input name='offgr' type='radio' value='0' />";} 	echo" &nbsp; &nbsp; ";
	if($offgr=="1"){echo"<input name='offgr' type='radio' value='1' checked='checked' />";} else {echo"<input name='offgr' type='radio' value='1'/>";}
	echo"Выкл<br/>";

echo"Смайлы:<br/>Вкл.";
	if($offsm=="0"){echo"<input name='offsm' type='radio' value='0' checked='checked'/>";} else {echo"<input name='offsm' type='radio' value='0' />";} 	echo" &nbsp; &nbsp; ";
	if($offsm=="1"){echo"<input name='offsm' type='radio' value='1' checked='checked' />";} else {echo"<input name='offsm' type='radio' value='1'/>";}
	echo"Выкл<br/>";

echo"Постраничный вывод:<br/>Вкл.";
	if($offpg=="0"){echo"<input name='offpg' type='radio' value='0' checked='checked'/>";} else {echo"<input name='offpg' type='radio' value='0' />";} 	echo" &nbsp; &nbsp; ";
	if($offpg=="1"){echo"<input name='offpg' type='radio' value='1' checked='checked' />";} else {echo"<input name='offpg' type='radio' value='1'/>";}
	echo"Выкл<br/>";

echo"Выбор транслита:<br/>Вкл.";
	if($offtr=="0"){echo"<input name='offtr' type='radio' value='0' checked='checked'/>";} else {echo"<input name='offtr' type='radio' value='0' />";} 	echo" &nbsp; &nbsp; ";
	if($offtr=="1"){echo"<input name='offtr' type='radio' value='1' checked='checked' />";} else {echo"<input name='offtr' type='radio' value='1'/>";}
	echo"Выкл<br/>";

echo"Быстрый переход:<br/>Вкл.";
	if($pereh=="0"){echo"<input name='pereh' type='radio' value='0' checked='checked'/>";} else {echo"<input name='pereh' type='radio' value='0' />";} 	echo" &nbsp; &nbsp; ";
	if($pereh=="1"){echo"<input name='pereh' type='radio' value='1' checked='checked' />";} else {echo"<input name='pereh' type='radio' value='1'/>";}
	echo"Выкл<br/>";
...
echo "<a href='usset.php?act=chat'>Настройки чата</a><br/>";
break;}} else {
require("../incfiles/head.php");
require("../incfiles/inc.php"); print "Ошибка!<br/>"; }
