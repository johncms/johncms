...
$idm = intval(check(trim($_POST['idm'])));
...
if (in_array($tfl,$df)){echo "Попытка отправить файл запрещенного типа.<br/><a href='pradd.php?act=write&amp;adr=".$adres."'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
if ($fsize >= 1024*$flsz){echo "Вес файла превышает $flsz кб<br/>
<a href='pradd.php?act=write&amp;adr=".$adres."'>Повторить</a><br/>";
require ('../incfiles/end.php');exit;}
if(eregi("[^a-z0-9.()+_-]",$fname)){echo"В названии файла <b>$fname</b> присутствуют недопустимые символы<br/>Разрешены только латинские символы, цифры и некоторые знаки ( .()+_- )<br/><a href='pradd.php?act=write&amp;adr=".$adres."'>Повторить</a><br/>";
require ('../incfiles/end.php');exit;}
...
<a href='pradd.php?act=write&amp;adr=".$adres."'>Повторить</a><br/>";
require ('../incfiles/end.php');exit;}
if(eregi("[^a-z0-9.()+_-]",$tmp_name)){echo"В названии файла <b>$tmp_name</b> присутствуют недопустимые символы<br/>Разрешены только латинские символы, цифры и некоторые знаки ( .()+_- )<br /><a href='pradd.php?act=write&amp;adr=".$adres."'>Повторить</a><br/>";
require ('../incfiles/end.php');exit;}
...
if (!empty($idm)){mysql_query("update `privat` set otvet='1' where id='".$idm."';");}
...
else {echo "Такого пользователя не существует<br/>";}}else {
...
if (in_array($tfl,$df)){
require("../incfiles/head.php");
require("../incfiles/inc.php");echo "Ошибка!<br/>&#187;<a href='pradd.php'>В приват</a><br/>";require ("../incfiles/end.php");exit;} 	 
...
require ("../incfiles/end.php");exit;}}else{$tema="Привет!";}
if (!empty($_GET['id'])){
$id=intval(check($_GET['id']));
...
echo "Вы не выбрали писем для удаления<br/><a href='pradd.php?act=in'>Назад</a><br/>";require("../incfiles/end.php");exit;}
...
echo "Новые входящие<br/>";}else{
$messages = mysql_query("select * from `privat` where user='".$login."' and type='in' order by time desc;");echo "<div style='text-align: center'>Входящие</div>";}
...
if (!empty($massiv[attach])){echo "+ вложение<br/>";}
if ($massiv[chit]=="no"){echo "Не прочитано<br/>";}
if ($massiv[otvet]==0){echo "Не отвечено<br/>";}
echo '</div>';}
++$i;}
...
if ($start != 0) {echo '<a href="pradd.php?act=in&amp;page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$count && $asd>0){echo ' <a href="pradd.php?act=in&amp;page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="pradd.php?act=in&amp;page='.$paa.'">'.$paa.'</a> <a href="pradd.php?act=in&amp;page='.($paa+1).'">'.($paa+1).'</a> .. <a href="pradd.php?act=in&amp;page='.($paa*2).'">'.($paa*2).'</a> <a href="pradd.php?act=in&amp;page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+$kmess;}
if ($page2>12){
echo ' .. <a href="pradd.php?act=in&amp;page='.$paa2.'">'.$paa2.'</a> <a href="pradd.php?act=in&amp;page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="pradd.php?act=in&amp;page='.($paa3).'">'.($paa3).'</a> <a href="pradd.php?act=in&amp;page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="pradd.php?act=in&amp;page='.$pa2.'">'.$pa2.'</a> <a href="pradd.php?act=in&amp;page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$count){echo ' .. <a href="pradd.php?act=in&amp;page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($count > $start + $kmess) {echo ' <a href="pradd.php?act=in&amp;page='.($page+1).'">&gt;&gt;</a>';}
...
$massiv1[text]=eregi_replace("\\<a href\\='((https?|ftp)://)([[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/%]*)?)?)'>[[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/%]*)?)?)</a>", "<a href='\\1\\3'>\\3</a>" ,$massiv1[text]);}else{
...
$tekst=smilesadm($tekst);}}else{$tekst=$massiv1[text];}
...
if (!empty($massiv[attach])){echo "+ вложение<br/>";}
if ($massiv[chit]=="no"){echo "Не прочитано<br/>";}
echo"<a href='pradd.php?act=delmess&amp;del=".$massiv[id]."'>Удалить</a></div>";}++$i;}
...
if ($start != 0) {echo '<a href="pradd.php?act=out&amp;page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$count && $asd>0){echo ' <a href="pradd.php?act=out&amp;page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
$pa2=$page+floor($page2/2);
...
$paa3=$page+(floor($page2/3)*2);
if ($page>13){
echo ' <a href="pradd.php?act=out&amp;page='.$paa.'">'.$paa.'</a> <a href="pradd.php?act=out&amp;page='.($paa+1).'">'.($paa+1).'</a> .. <a href="pradd.php?act=out&amp;page='.($paa*2).'">'.($paa*2).'</a> <a href="pradd.php?act=out&amp;page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+$kmess;}
if ($page2>12){
echo ' .. <a href="pradd.php?act=out&amp;page='.$paa2.'">'.$paa2.'</a> <a href="pradd.php?act=out&amp;page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="pradd.php?act=out&amp;page='.($paa3).'">'.($paa3).'</a> <a href="pradd.php?act=out&amp;page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="pradd.php?act=out&amp;page='.$pa2.'">'.$pa2.'</a> <a href="pradd.php?act=out&amp;page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$count){echo ' .. <a href="pradd.php?act=out&amp;page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($count > $start + $kmess) {echo ' <a href="pradd.php?act=out&amp;page='.($page+1).'">&gt;&gt;</a>';}
