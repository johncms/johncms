...
echo '<div class="phdr"><a href="?act=smilies"><b>' . _t('Smilies') . '</b></a> | ' . _td('For administration', 'help') . '</div>';
...
echo '<div class="topmenu"><a href="?act=my_smilies">' . _td('My smilies', 'help') . '</a>  (' . count($user_sm) . ' / ' . $user_smileys . ')</div>' .
...
if ($total > 0) {
    ...
    for ($i = $start; $i < $end; $i++) {
        ...
        echo $smileys . '<img src="../images/smileys/admin/' . $array[$i] . '" alt="" /> - :' . $smile . ': ' . _td('or', 'help') . ' :' . functions::trans($smile) . ':</div>';
    }
} else {
