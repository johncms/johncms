...
require("../incfiles/head.php");
require("../incfiles/inc.php");echo"У ВАС НЕДОСТАТОЧНО ПРАВ ДЛЯ ЭТОГО!<br/>";require ("../incfiles/end.php");exit;}
...
require ("../incfiles/end.php");exit;}
if ($dostadm==1){
if (empty($vrem)){$ban=2;}else{
$ban=1;}}else{$ban=1;
if (empty($vrem)||$vrem>2880){$vrem=2880;}}
$vrem=round($realtime+($vrem*60));
...
mysql_query("insert into `bann` values('".$nam."','','','".$login."', '".$realtime."','".$whyban."','yes','".$mod."');");}else{
...
if($nam==$login){echo'С УМА СОШЁЛ?САМОГО СЕБЯ В БАНЮ?<br/>';}
...
if (!empty($nme[why])){echo ": $nme[why]<br/>";}else{ echo "не указана<br/>";}
if($nme[ban]=="2"){echo"Бан активен до отмены<br/>";}
if($nme[ban]=="1"){
$tti=round(($nme[bantime]-$realtime)/60);
if($tti>60) {$jjtti=round($tti/60).' час.';}else{$jjtti=$tti.' мин';}
...
if (!empty($nme[fwhy])){echo ": $nme[fwhy]<br/>";}else{ echo "не указана<br/>";}
...
if (!empty($nme[chwhy])){echo ": $nme[chwhy]<br/>";}else{ echo "не указана<br/>";}
...
if ($dostfmod==1&&($nme[fban]!=1||$nme[ftime]<$realtime)){echo "<option value='1'>Пнуть из форума</option>";}
if ($dostcmod==1&&($nme[chban]!=1||$nme[chtime]<$realtime)){echo "<option value='2'>Пнуть из чата</option>";}
if ($dostkmod==1&&($nme[ban]!=1||$nme[bantime]<$realtime)&&$nme[ban]!=2){echo "<option value='3'>Забанить</option>";}

echo "</select><br/>На какое время в минутах:<br/>";
if ($dostadm==1){
echo "(Если не указывать,бан даётся до отмены.<br/>";}elseif($dostkmod==1){
echo "(Макс. время бана 48 часов.<br/>";}else{echo "(";}
