...
$md1=mysql_fetch_array($md);
$to=$md1[avtor];
echo "Пишем в модерку для $to ";}else{
...
echo "<a href='moderka.php?act=add&amp;id=".$mass[id]."'>$mass[avtor]</a>";}else{
echo "$mass[avtor]";}
...
if ($start != 0) {echo '<a href="moderka.php?page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$count && $asd>0){echo ' <a href="moderka.php?page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="moderka.php?page='.$paa.'">'.$paa.'</a> <a href="moderka.php?page='.($paa+1).'">'.($paa+1).'</a> .. <a href="moderka.php?page='.($paa*2).'">'.($paa*2).'</a> <a href="moderka.php?page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+$kmess;}
if ($page2>12){
echo ' .. <a href="moderka.php?page='.$paa2.'">'.$paa2.'</a> <a href="moderka.php?page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="moderka.php?page='.($paa3).'">'.($paa3).'</a> <a href="moderka.php?page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="moderka.php?page='.$pa2.'">'.$pa2.'</a> <a href="moderka.php?page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$count){echo ' .. <a href="moderka.php?page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($count > $start + $kmess) {echo ' <a href="moderka.php?page='.($page+1).'">&gt;&gt;</a>';}
