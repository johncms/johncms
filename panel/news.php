...
if (empty($_POST['name'])){
echo "Вы не ввели заголовок<br/><a href='news.php?act=new'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
if (empty($_POST['text'])){
echo "Вы не ввели текст<br/><a href='news.php?act=new'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
...
case "edit":
if (empty($_GET['id'])){ 
echo "Ошибка!<br/><a href='news.php'>К новостям</a><br>";require ("../incfiles/end.php");exit;}
$id=intval(check($_GET['id']));
if (isset($_POST['submit'])){

if (empty($_POST['name'])){
echo "Вы не ввели заголовок<br/><a href='news.php?act=edit&amp;id=".$id."'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
if (empty($_POST['text'])){
echo "Вы не ввели текст<br/><a href='news.php?act=edit&amp;id=".$id."'>Повторить</a><br/>";require ("../incfiles/end.php");exit;}
$name=check($_POST['name']);
$text=check($_POST['text']);

mysql_query("update `news` set name='".$name."', text='".$text."' where id='".$id."';");
echo "Новость изменена.<br/><a href='news.php'>Продолжить</a><br/>";}else{
$n = mysql_query("select * from `news` where id='".$id."';");
$n1=mysql_fetch_array($n);
echo "Редактирование новости<br/><form action='news.php?act=edit&amp;id=".$id."' method='post'>Заголовок:<br/><input type='text' name='name' value='".$n1[name]."'/><br/>Текст:<br/><input type='text' name='text' value='".$n1[text]."'/><br/><input type='submit' name='submit' value='Ok!'/></form><br/><a href='news.php'>К новостям</a><br/>";}
...
echo "База новостей очищена!<br/><a href='news.php'>К новостям</a><br/>";}else{echo "Вы уверены,что хотите удалить все новости?<br/><a href='news.php?act=del&amp;all&amp;yes'>Да</a> | <a href='news.php'>Нет</a><br/>";}}else{
echo "Ошибка!<br/><a href='news.php'>К новостям</a><br>";require ("../incfiles/end.php");exit;}}else{
$id=intval(check($_GET['id']));
if (isset($_GET['yes'])){
mysql_query("delete from `news` where `id`='".$id."';");
echo "Новость удалена!<br/><a href='news.php'>К новостям</a><br/>";}else{echo "Вы уверены,что хотите удалить новость?<br/><a href='news.php?act=del&amp;id=".$id."&amp;yes'>Да</a> | <a href='news.php'>Нет</a><br/>";}}
...
echo "<a href='../forum/?id=".$nw1[kom]."'>Комментарии ($komm)</a><br/>";}else{echo "Новость не нуждается в комментариях<br/>";}
