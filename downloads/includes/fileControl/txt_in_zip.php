...
if (!$req_down->rowCount() || !is_file($res_down['dir'] . '/' . $res_down['name']) || (pathinfo($res_down['name'], PATHINFO_EXTENSION) != 'txt' && !isset($_GET['more'])) || ($res_down['type'] == 3 && $systemUser->rights < 6 && $systemUser->rights != 4)) {
...
if (isset($_GET['more'])) {
    ...
    if (!$req_more->rowCount() || !is_file($res_down['dir'] . '/' . $res_more['name']) || pathinfo($res_more['name'], PATHINFO_EXTENSION) != 'txt') {
        echo _t('File not found') . ' <a href="?">' . _t('Downloads') . '</a>';
        exit;
    }
