...
class getid3_mp3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        $CurrentDataLAMEversionString = (! empty($CurrentDataLAMEversionString) ? $CurrentDataLAMEversionString : ($info['audio']['encoder'] ?? ''));
            ...
            switch ($thisfile_mpeg_audio['channelmode']) {
                ...
                case 'dual channel':
                    if (($thisfile_mpeg_audio['bitrate'] == 'free') || ($thisfile_mpeg_audio['bitrate'] == 64000) || ($thisfile_mpeg_audio['bitrate'] >= 96000)) {
                        ...
                        $info['error'][] = (int) (round($thisfile_mpeg_audio['bitrate'] / 1000)) . 'kbps not allowed in Layer 2, ' . $thisfile_mpeg_audio['channelmode'] . '.';
            ...
            if ((substr($headerstring, $VBRidOffset, strlen('Xing')) == 'Xing') || (substr($headerstring, $VBRidOffset, strlen('Info')) == 'Info')) {
                ...
                if (! empty($thisfile_mpeg_audio['VBR_frames'])) {
                    ...
                    if (! empty($thisfile_mpeg_audio['VBR_bytes'])) {
                        ...
                        $used_filesize -= (int) (@$info['id3v2']['headerlength']);
        ...
        if (($thisfile_mpeg_audio['bitrate'] == 'free') && empty($info['audio']['bitrate'])) {
            if (($offset == $info['avdataoffset']) && empty($thisfile_mpeg_audio['VBR_frames'])) {
                ...
                if ($framebytelength > 0) {
                    ...
                    if ($thisfile_mpeg_audio['layer'] == '1') {
                        // BitRate = (((FrameLengthInBytes / 4) - Padding) * SampleRate) / 12
                        $info['audio']['bitrate'] = ((($framebytelength / 4) - (int) ($thisfile_mpeg_audio['padding'])) * $thisfile_mpeg_audio['sample_rate']) / 12;
                    } else {
                        // Bitrate = ((FrameLengthInBytes - Padding) * SampleRate) / 144
                        $info['audio']['bitrate'] = (($framebytelength - (int) ($thisfile_mpeg_audio['padding'])) * $thisfile_mpeg_audio['sample_rate']) / 144;
                    }
                } else {
    ...
    public function FreeFormatFrameLength($offset, $deepscan=false)
    {
        ...
        if ($deepscan) {
            ...
            if (count($ActualFrameLengthValues) > 0) {
                $framelength = (int) (round(array_sum($ActualFrameLengthValues) / count($ActualFrameLengthValues)));
            }
        }
                ...
                if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
                    ...
                    if ($next4[0] == "\xFF") {
                        ...
                        if ($MPEGaudioHeaderValidCache[$next4]) {
                            ...
                            getid3_lib::safe_inc($Distribution['padding'][(int) ($LongMPEGpaddingLookup[$head4])]);
    ...
    public static function ClosestStandardMP3Bitrate($bit_rate)
    {
        ...
        $round_bit_rate = (int) (round($bit_rate, -3));
    ...
    public static function LAMEvbrMethodLookup($VBRmethodID)
    {
        ...
        return $LAMEvbrMethodLookup[$VBRmethodID] ?? '';
    }
    ...
    public static function LAMEmiscStereoModeLookup($StereoModeID)
    {
        ...
        return $LAMEmiscStereoModeLookup[$StereoModeID] ?? '';
    }
    ...
    public static function LAMEmiscSourceSampleFrequencyLookup($SourceSampleFrequencyID)
    {
        ...
        return $LAMEmiscSourceSampleFrequencyLookup[$SourceSampleFrequencyID] ?? '';
    }
    ...
    public static function LAMEsurroundInfoLookup($SurroundInfoID)
    {
        ...
        return $LAMEsurroundInfoLookup[$SurroundInfoID] ?? 'reserved';
    }
    ...
    public static function LAMEpresetUsedLookup($LAMEtag)
    {
        ...
        return $LAMEpresetUsedLookup[$LAMEtag['preset_used_id']] ?? 'new/unknown preset: ' . $LAMEtag['preset_used_id'] . ' - report to info@getid3.org';
    }
}
