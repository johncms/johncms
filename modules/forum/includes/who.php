...
if ($id) {
    ...
    if ($req->rowCount()) {
        ...
        if ($total) {
            $req = $db->query('SELECT * FROM `' . ($do == 'guest' ? 'cms_sessions' : 'users') . '` WHERE `lastdate` > ' . (time() - 300) . " AND `place` LIKE 'forum,${id},topic' ORDER BY " . ($do == 'guest' ? '`movings` DESC' : '`name` ASC') . " LIMIT ${start}, " . $user->config->kmess);
                        ...
                        switch ($where[2]) {
                            ...
                            case 'message':
                                ...
                                if (! empty($message)) {
                                    ...
                                    if ($req_m->rowCount()) {
                                        $res_m = $req_m->fetch();
                                        $place = (isset($where[2]) ? _t('Answers in the Topic') : _t('In the Topic')) . ' &quot;<a href="?type=topic&id=' . $res_m['id'] . '">' . (empty($res_m['name']) ? '-----' : $res_m['name']) . '</a>&quot;';
                                    }
                                }
                                break;
                        }
