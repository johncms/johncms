...
if ($dostsadm==1){
$psk = mysql_query("select * from `forum` where type='t' and moder='1' order by time desc ;");}else{
$psk = mysql_query("select * from `forum` where type='t' and moder='1' and close!='1' order by time desc ;");}
}else{
if ($dostsadm==1){
$psk = mysql_query("select * from `forum` where type='t' and moder='1' order by time ;");}else{
...
if ($array[vip]==1){$hd="<img src='../images/pt.gif' alt=''/>";}
elseif ($array[edit]==1){$hd="<img src='../images/tz.gif' alt=''/>";}
else {
$np = mysql_query("select * from `forum` where type='l' and time>'".$array[time]."' and refid='".$array[id]."' and `from`='".$login."';");
$np1 = mysql_num_rows($np);
if ($np1==0){
$hd="<img src='../images/np.gif' alt=''/>";}else{
$hd="<img src='../images/op.gif' alt=''/>";}}
if ($array[close]==1){$tst="<font color='#FFF000'>Тема удалена!</font><br/>";}else{$tst="";}
...
if ((!empty($_SESSION['uppost']))&&($_SESSION['uppost']==1)){
if ($dostsadm==1){
$psk = mysql_query("select * from `forum` where type='m' order by time desc ;");}else{
$psk = mysql_query("select * from `forum` where type='m' and close!='1' order by time desc ;");}
}else{
if ($dostsadm==1){
$psk = mysql_query("select * from `forum` where type='m' order by time ;");}else{
...
if ((!empty($_SESSION['uppost']))&&($_SESSION['uppost']==1)){
if ($dostsadm==1){
$q1 = mysql_query("select * from `forum` where type='m' and refid='".$tem1[id]."'  order by time desc ;");}else{
$q1 = mysql_query("select * from `forum` where type='m' and close!='1' and refid='".$tem1[id]."'  order by time desc ;");}
$tp=1;
}else{
if ($dostsadm==1){
$q1 = mysql_query("select * from `forum` where type='m' and refid='".$tem1[id]."'  order by time ;");}else{
...
if ($array[close]==1){$hd="$hd <font color='#FFF000'>Пост удалён!</font><br/>";}
if (!empty($array[to])){$hd="$hd $array[to], ";}
...
$array[text]=eregi_replace("\\<a href\\='((https?|ftp)://)([[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/;%]*)?)?)'>[[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/;%]*)?)?)</a>", "<a href='\\1\\3'>\\3</a>" ,$array[text]);}else{
...
$tekst=smilesadm($tekst);}}else{$tekst=$array[text];}
...
##
if ($tem1[vip]==1){$tinf="<img src='../images/pt.gif' alt=''/>";}
elseif ($tem1[edit]==1){$tinf="<img src='../images/tz.gif' alt=''/>";}
else {
$np = mysql_query("select * from `forum` where type='l' and time>'".$tem1[time]."' and refid='".$tem1[id]."' and `from`='".$login."';");
$np1 = mysql_num_rows($np);
if ($np1==0){
$tinf="<img src='../images/np.gif' alt=''/>";}else{
$tinf="<img src='../images/op.gif' alt=''/>";}}
if ($tem1[close]==1){$tst="<font color='#FFF000'>Тема удалена!</font><br/>";}else{$tst="";}
...
if ($start != 0) {echo '<a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$g && $asd>0){echo ' <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.$paa.'">'.$paa.'</a> <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($paa+1).'">'.($paa+1).'</a> .. <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($paa*2).'">'.($paa*2).'</a> <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+$kmess;}
if ($page2>12){
echo ' .. <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.$paa2.'">'.$paa2.'</a> <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($paa3).'">'.($paa3).'</a> <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.$pa2.'">'.$pa2.'</a> <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$g){echo ' .. <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}

if ($g > $start + $kmess) {echo ' <a href="search.php?act=go&amp;m='.$m.'&amp;srh='.$srh.'&amp;page='.($page+1).'">&gt;&gt;</a>';}
echo "<form action='search.php' method='get'>Перейти к странице:<br/><input type='hidden' name='m' value='".$m."'/><input type='hidden' name='act' value='go'/><input type='hidden' name='srh' value='".$srh."'/><input type='text' name='page' title='Введите номер страницы'/><br/><input type='submit' title='Нажмите для перехода' value='Go!'/></form>";}
