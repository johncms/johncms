...
if($datauser[fwhy]==""){echo"<div>Причина не указана</div>";}else{echo"Причина:<font color='red'> $datauser[fwhy]</font><br>";}
...
if ($wherf1[0]=="forum"){$count=$count+1;}}
...
echo "$div";
if (empty($wher1[1])){$adres="<a href='index.php'>На главной форума</a>";}else{
...
if ($tem[close]==1){$adres="<a href='index.php'>На главной форума</a>";}else{
...
$q4 = mysql_query("select * from `forum` where type='f' and id='".$razd[refid]."';");
$frm=mysql_fetch_array($q4);
if ($adr1[close]==1){$adres="<a href='index.php'>На главной форума</a>";}else{
...
if ($start != 0) {echo '<a href="who.php?id='.$id.'&amp;page='.($page-1).'">&lt;&lt;</a> ';}
if ($offpg!=1){
if($asd<$count && $asd>0){echo ' <a href="who.php?id='.$id.'&amp;page=1&amp;">1</a> .. ';}
$page2=$ba-$page;
$pa=ceil($page/2);
$paa=ceil($page/3);
...
if ($page>13){
echo ' <a href="who.php?id='.$id.'&amp;page='.$paa.'">'.$paa.'</a> <a href="who.php?id='.$id.'&amp;page='.($paa+1).'">'.($paa+1).'</a> .. <a href="who.php?id='.$id.'&amp;page='.($paa*2).'">'.($paa*2).'</a> <a href="who.php?id='.$id.'&amp;page='.($paa*2+1).'">'.($paa*2+1).'</a> .. ';}
elseif ($page>7){
...
$i=$i+10;}
if ($page2>12){
echo ' .. <a href="who.php?id='.$id.'&amp;page='.$paa2.'">'.$paa2.'</a> <a href="who.php?id='.$id.'&amp;page='.($paa2+1).'">'.($paa2+1).'</a> .. <a href="who.php?id='.$id.'&amp;page='.($paa3).'">'.($paa3).'</a> <a href="who.php?id='.$id.'&amp;page='.($paa3+1).'">'.($paa3+1).'</a> ';}
elseif ($page2>6){
echo ' .. <a href="who.php?id='.$id.'&amp;page='.$pa2.'">'.$pa2.'</a> <a href="who.php?id='.$id.'&amp;page='.($pa2+1).'">'.($pa2+1).'</a> ';}
if($asd2<$count){echo ' .. <a href="who.php?id='.$id.'&amp;page='.$ba.'">'.$ba.'</a>';}}else{
echo "<b>[$page]</b>";}


if ($count > $start + 10) {echo ' <a href="who.php?id='.$id.'&amp;page='.($page+1).'">&gt;&gt;</a>';}
echo "<form action='pradd.php'>Перейти к странице:<br/><input type='hidden' name='id' value='".$id."'/><input type='text' name='page' title='Введите номер страницы'/><br/><input type='submit' title='Нажмите для перехода' value='Go!'/></form>";}
